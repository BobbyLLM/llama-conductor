...
```text
Vodka CTC keeps long chats manageable:
- keeps prompt growth bounded with breadcrumb-backed notes on disk
...
- configurable compaction pressure via `compaction.compaction_pressure_pct`
- user definable presets (fast / balanced / max-recall)
```

Result: 
- Consistent prompt size, stable performance, and configurable compaction pressure (stdlib deterministic, no LLM compute).
...
### 10) 📚 Codex grounded wiki (because retraining is not a hobby)

WITHOUT llama-conductor:
```text
Model knows what it knows. You know it's wrong about your thing.
Your options: prompt harder, add context every session, accept the hallucination,
or fine-tune a model.
Model: [confidently wrong about your specific domain, every time]
You: I've told you this three times
Model: Understood. [wrong again]
```

WITH llama-conductor:
```text
You add the correct information to a knowledge base folder. Codex ingests it.
Router:
- deterministic Python tooling extracts structure from your source material
- no LLM summary pass — extractive, not generative
- builds an inspectable on-disk wiki that updates when your material changes
- retrieval order: Cheatsheets → Codex → >>wiki → >>web → model fallback
- if the answer is in your material, it comes from your material
- if it isn't, the router says so and moves to the next layer
```

Result:
- You've updated the model's effective knowledge on that topic. No weights touched.
- It answers from what you wrote, not what it was trained to guess.
- Add new material → Codex updates → model knows the new thing. Same mechanism, every time.
- Teach it once and reboot. From that point forward, it will know.
- If Codex doesn't have it, it doesn't hallucinate. It falls through cleanly.

In short: Codex is how you teach the model without retraining it.
...
- Keeps the live chat from ballooning
- Keeps turn-time behavior stable on modest hardware
- Preserves usable memory with breadcrumb-backed notes on disk
- Compaction pressure is configurable from `router_config.yaml` (`compaction.compaction_pressure_pct`)
