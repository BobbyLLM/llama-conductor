...
from ..vodka_filter import COMPACTION_PRESSURE_PCT
...
try:
    from ..cheatsheets_runtime import _build_web_facts as _build_web_facts_block  # type: ignore
except Exception:
    _build_web_facts_block = None  # type: ignore
...
try:
    from ..vodka_filter import purge_session_memory_jsonl, purge_ctc_compaction_jsonl, VodkaFilter  # type: ignore
except Exception:
    purge_session_memory_jsonl = None  # type: ignore
    purge_ctc_compaction_jsonl = None  # type: ignore
...
def _ctc_compaction_status_snapshot(session_id: str, state: SessionState) -> Dict[str, object]:
    out: Dict[str, object] = {
        "raw_threshold_pct": "n/a",
        "threshold_pct": float(COMPACTION_PRESSURE_PCT),
        "current_pressure_pct": "n/a",
        "turn_backstop_every": 12,
        "turn_backstop_remaining": "n/a",
        "record_count": "n/a",
        "last_trigger": "n/a",
        "last_ctx_pressure_pct": "n/a",
    }
    try:
        if state.vodka and hasattr(state.vodka, "get_ctc_compaction_status"):
            payload = state.vodka.get_ctc_compaction_status(
                session_id=session_id,
                user_turn_hint=int(getattr(state, "profile_turn_counter", 0) or 0),
            )
            out["raw_threshold_pct"] = payload.get("raw_threshold_pct", "n/a")
            out["threshold_pct"] = float(payload.get("threshold_pct", COMPACTION_PRESSURE_PCT) or COMPACTION_PRESSURE_PCT)
            out["current_pressure_pct"] = float(
                payload.get("current_pressure_pct", getattr(state, "ctc_current_prompt_pressure_pct", "n/a")) or getattr(state, "ctc_current_prompt_pressure_pct", "n/a")
            )
            out["turn_backstop_every"] = int(payload.get("turn_backstop_every", 12) or 12)
            out["turn_backstop_remaining"] = payload.get("turn_backstop_remaining", "n/a")
            out["record_count"] = int(payload.get("record_count", 0) or 0)
            out["last_trigger"] = str(payload.get("last_trigger", "") or "")
            out["last_ctx_pressure_pct"] = float(payload.get("last_ctx_pressure_pct", 0.0) or 0.0)
    except Exception:
        pass
    if out.get("current_pressure_pct") == "n/a":
        try:
            current = float(getattr(state, "ctc_current_prompt_pressure_pct", "n/a"))
            out["current_pressure_pct"] = current
        except Exception:
            pass
    return out


def _render_compaction_brief(ctc: Dict[str, object]) -> str:
    threshold_raw = ctc.get("raw_threshold_pct")
    threshold_applied = ctc.get("threshold_pct")
    pressure = ctc.get("current_pressure_pct")

    try:
        applied = float(threshold_applied)
    except Exception:
        return "Compaction threshold: n/a | Current pressure: n/a | Headroom: n/a"

    try:
        raw_val = float(threshold_raw)
    except Exception:
        raw_val = None

    try:
        pressure_val = float(pressure)
        headroom_val = max(0.0, applied - pressure_val)
        pressure_text = f"{pressure_val:.0f}%"
        headroom_text = f"{headroom_val:.0f}%"
    except Exception:
        pressure_text = "n/a"
        headroom_text = "n/a"

    threshold_text = f"{applied:.0f}%"
    if raw_val is not None and abs(raw_val - applied) > 1e-9:
        threshold_text = f"{threshold_text} (config, clamped from {raw_val:.0f}%)"
    else:
        threshold_text = f"{threshold_text} (config)"
    return f"Compaction threshold: {threshold_text} | Current pressure: {pressure_text} | Headroom: {headroom_text}"


def _render_compaction_debug_line(state: SessionState) -> str:
    meta = dict(getattr(state, "turn_compaction_debug", {}) or {})
    if not meta:
        return "ctc_compaction_debug=n/a"
    return (
        "ctc_compaction_debug="
        f"ingestion={int(bool(meta.get('ingestion_turn')))} "
        f"snapshot_first={int(bool(meta.get('snapshot_first_mode')))} "
        f"gate={int(bool(meta.get('gate_active')))} "
        f"snapshot={int(bool(meta.get('snapshot_active')))} "
        f"attempted={int(bool(meta.get('lookup_attempted')))} "
        f"found={int(bool(meta.get('lookup_found')))} "
        f"source={str(meta.get('lookup_source', '') or '')} "
        f"field={str(meta.get('lookup_field', '') or '')} "
        f"reason={str(meta.get('lookup_reason', '') or '')} "
        f"record_count={int(meta.get('snapshot_record_count', 0) or 0)} "
        f"snapshot_id={str(meta.get('snapshot_last_id', '') or '')}"
    )
...
def _render_status_raw(session_id: str, state: SessionState) -> str:
    ...
    ctc = _ctc_compaction_status_snapshot(session_id, state)
    return (
        "[status]\n"
        ...
        f"{_render_compaction_brief(ctc)}\n"
        f"{_render_compaction_debug_line(state)}\n"
    )
...
def _render_status(session_id: str, state: SessionState) -> str:
    ...
    return (
        ...
        f"- `Compaction`: pressure_pct=`{ctc.get('threshold_pct')}` | backstop_remaining=`{ctc.get('turn_backstop_remaining')}` | last_trigger=`{ctc.get('last_trigger')}` | last_pressure_pct=`{ctc.get('last_ctx_pressure_pct')}`\n"
        f"- `CTC Compaction`: {_render_compaction_debug_line(state)}\n"
...
def _render_status_full(session_id: str, state: SessionState) -> str:
    ...
    return (
        ...
        "\n"
        "## CTC Compaction\n"
        f"- `pressure_pct`: `{ctc.get('threshold_pct')}`\n"
        f"- `raw_pressure_pct`: `{ctc.get('raw_threshold_pct')}`\n"
        f"- `debug`: `{_render_compaction_debug_line(state)}`\n"
        f"- `turn_backstop_every`: `{ctc.get('turn_backstop_every')}`\n"
        f"- `turn_backstop_remaining`: `{ctc.get('turn_backstop_remaining')}`\n"
        f"- `last_trigger`: `{ctc.get('last_trigger')}`\n"
        f"- `last_pressure_pct`: `{ctc.get('last_ctx_pressure_pct')}`\n"
        "\n"
        "## Retrieval\n"
...
def handle_command(cmd_text: str, *, state: SessionState, session_id: str) -> Optional[str]:
    ...
    if cmd_key == "profile":
        ...
        if sub in ("direct", "neutral", "softened"):
            ...
            if ok and sub == "softened":
                profile_set(state.interaction_profile, "sarcasm_level", "low")
                profile_set(state.interaction_profile, "snark_tolerance", "low")
    ...
    if low == "flush":
        ...
        state.last_flush_message_index = getattr(state, "_flush_message_count", 0)
        try:
            del state._flush_message_count
        ...
        state.last_web_results_load_id = ""
        state.last_web_facts_block = ""
        state.last_web_facts_turn = 0
        ...
        if purge_ctc_compaction_jsonl:
            try:
                ...
                if state.vodka is not None and hasattr(state.vodka, "valves"):
                    base = str(getattr(state.vodka.valves, "storage_dir", "") or "").strip()
                    subdir = str(getattr(state.vodka.valves, "subdir", subdir) or subdir).strip()
                compaction_purged = int(purge_ctc_compaction_jsonl(base, vodka_subdir=subdir))
            except Exception:
                compaction_purged = 0
        ...
        if clear_scratchpad:
            try:
                ...
                scratch_cleared = False
        state.scratchpad_locked_indices.clear()
        state.last_web_assistant_source = ""
        ...
        if not flush_ctc_cache or not state.vodka:
            return (
                ...
                f"(no Vodka cache initialized yet). Other reset actions completed. Session-memory deleted={purged}. Compaction deleted={compaction_purged}.\n"
        ...
        return (
            ...
            f"[flush] compaction deleted={compaction_purged}.\n"
            f"[flush] scratchpad deleted={scratch_deleted} cleared={str(scratch_cleared).lower()} lock=cleared.\n"
            f"[flush] judge-audit deleted={judge_deleted} ({judge_audit_dir}).{_dm_line}"
        )
    ...
    if cmd_key == "web":
            ...
            try:
                ...
                if ok and isinstance(evidence, dict):
                    ...
                    if isinstance(rows, list):
                        ...
                        state.last_web_results_load_id = ""
                    if _build_web_facts_block is not None:
                        try:
                            state.last_web_facts_block = str(_build_web_facts_block(evidence) or "").strip()
                            state.last_web_facts_turn = int(getattr(state, "profile_turn_counter", 0) or 0)
        ...
        if _is_sidecar_success(raw, "[web]"):
            state.turn_web_evidence_injected = state.last_turn_web_evidence_injected = True
            state.last_web_assistant_source = "Web"
            return _append_command_footer(raw, confidence="medium", source="Web")
