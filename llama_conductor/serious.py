...
- Default tone is neutral unless CONSTRAINTS specify otherwise.
...
def _extract_chat_summary(messages: List[Dict[str, Any]]) -> str:
    ...
    for m in reversed(messages or []):
        if m.get("role") == "system":
            ...
            if isinstance(c, str):
                cs = c.strip()
                if cs.startswith("[CHAT_SUMMARY]") or cs.startswith("[SESSION_MEMORY]") or cs.startswith("[CTC_COMPACTION]"):
                    return cs
    return ""
...
def run_serious(
    ...
    *,
    ...
    body = {"messages": history, "session_id": session_id, "ctx_size": int(ctx_size or 0)}
    try:
    ...
    priority_line = (
        "CONSTRAINT PRIORITY:\n"
        "- If CONSTRAINTS specify tone/register/style, those constraints override default tone guidance.\n"
    )
