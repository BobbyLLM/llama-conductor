...
- `>>web synth <query>` evidence-gated web synthesis (refuses if evidence is weak)
...
- `>>flush` clear CTC compaction state/history cache, reset session profile/style identity, and delete session-memory, scratchpad session captures, plus judge-audit JSONL files (does not detach KBs)
- `>>vodka debug` show Vodka trim-event log for this session
Tips:
...
- `>>scratch online <query>` interactive retrieval-to-scratch lane (`web` / `wiki` / `web synth`)
- `>>scratch online web <query>` deterministic online load from web listing
- `>>scratch online wiki <query>` deterministic online load from wiki
- `>>scratch online web synth <query>` deterministic online load from synth output
- `>>scratch load web` load last `>>web` snapshot into scratch (idempotent per snapshot)
...
- `!! nuke` clears Vodka memory only; `>>flush` clears CTC compaction state and resets profile/style runtime state.
