...
from .nl_search_alias import is_bare_nl_search_alias, summarize_nl_search_alias_response
...
_INLINE_SIDECAR_CMD_RE = re.compile(
    r"(?is)(?:^|[\s,;])"
    r"(>>\s*(?:web(?:\s+synth)?|wiki|define)\b.+)$"
)
...
def _extract_inline_sidecar_command(user_text_raw: str) -> str:
    raw = str(user_text_raw or "").strip()
    if not raw or raw.startswith(">>"):
        return ""
    m = _INLINE_SIDECAR_CMD_RE.search(raw)
    if not m:
        return ""
...
def handle_preflight(
    *,
    ...
    if selector == "":
        inline_cmd = _extract_inline_sidecar_command(user_text_raw)
        if inline_cmd:
            user_text_raw = inline_cmd
            selector, user_text = split_selector(user_text_raw)
            for i in range(len(raw_messages) - 1, -1, -1):
                if raw_messages[i].get("role") == "user":
                    raw_messages[i]["content"] = user_text_raw
                    break
        if str(user_text_raw or "").strip().lower() in (">>flush", "flush"):
            state._flush_message_count = len(raw_messages)
        try:
        ...
        if alias_cmd:
            ...
            if alias_reply is not None:
                text = alias_reply
                if is_bare_nl_search_alias(user_text_raw) and str(alias_cmd or "").startswith(">>web "):
                    try:
                        query = str(alias_cmd[len(">>web "):] or "").strip()
                        text = summarize_nl_search_alias_response(state=state, query=query, raw_reply=text)
                    except Exception:
                        pass
                if maybe_capture_command_output is not None:
                    try:
