...
_SHORT_TRIVIA_TOKEN_PENALTY_THRESHOLD = 12
_SHORT_TRIVIA_PENALTY = 0.60
_REFERENCE_SECTION_PENALTY = 0.30
_REFERENCE_SECTION_RE = re.compile(
    r"\b(?:see also|external links|references|further reading|notes|compatibility|supported|list of)\b",
    re.I,
)
_STOPWORDS = {
...
def _paragraph_heading_level(paragraph: str) -> int:
    ...
    if re.match(r"^##\s+", s):
        return 2
    # HTML/wiki/plain-text extracts often degrade headings into standalone title lines.
    if "\n" not in s and _is_header_like(s):
        return 1
    return 0


def _looks_infobox_or_table_paragraph(paragraph: str) -> bool:
    s = (paragraph or "").strip()
    if not s:
        return False
    if "|" in s:
    ...
    for line in s.splitlines():
        ...
        if not line:
            continue
        if re.match(r"^[A-Z][A-Za-z0-9 /()'&,.-]{1,40}(?:\t| {2,}|:\s)\S", line):
            return True
    return False


def _paragraph_has_trailing_heading_line(paragraph: str) -> bool:
    lines = [ln.strip() for ln in (paragraph or "").splitlines() if ln and ln.strip()]
    if not lines:
        return False
    last = lines[-1]
    if last.endswith((".", "!", "?", ":", ";")):
        return False
    return _is_header_like(last)


def _paragraph_trailing_heading_text(paragraph: str) -> str:
    lines = [ln.strip() for ln in (paragraph or "").splitlines() if ln and ln.strip()]
    if not lines:
        return ""
    last = lines[-1]
    if last.endswith((".", "!", "?", ":", ";")):
        return ""
    return last if _is_header_like(last) else ""


def _heading_section_penalty(heading_text: str) -> float:
    s = str(heading_text or "").strip()
    if not s:
        return 1.0
    return _REFERENCE_SECTION_PENALTY if _REFERENCE_SECTION_RE.search(s) else 1.0


def _dedupe_near_duplicate_sentences(sentences: List[str], multipliers: List[float], threshold: float = 0.60) -> tuple[List[str], List[float]]:
    kept_sentences: List[str] = []
    kept_multipliers: List[float] = []
    ...
    for idx, sent in enumerate(sentences):
        ...
        if toks:
            for prev_toks in kept_token_sets:
                if not prev_toks:
                    continue
                overlap = len(toks & prev_toks) / max(1, len(toks | prev_toks))
        ...
        kept_multipliers.append(multipliers[idx])
        kept_token_sets.append(toks)
    return kept_sentences, kept_multipliers
...
def _extractive_sentences(text: str) -> List[str]:
    ...
    section_penalties: List[float] = []
    seen: Set[str] = set()
    has_headings = any(_paragraph_heading_level(p) > 0 for p in raw_paragraphs)
    heading_seen = False
    lead_assigned = False
    section_open_remaining = 0
    current_section_penalty = 1.0

    for paragraph in raw_paragraphs:
        heading_level = _paragraph_heading_level(paragraph)
        if heading_level > 0:
            heading_seen = True
            section_open_remaining = 4
            current_section_penalty = _heading_section_penalty(paragraph)
            continue
        infobox_like = _looks_infobox_or_table_paragraph(paragraph)
        trailing_heading = _paragraph_has_trailing_heading_line(paragraph)
        trailing_heading_text = _paragraph_trailing_heading_text(paragraph)
        paragraph_sentences: List[str] = []
        paragraph_seen_norms: List[str] = []
        for sent in _split_sentences(paragraph):
            ...
            if norm in seen:
                continue
            paragraph_sentences.append(s)
            paragraph_seen_norms.append(norm)
        multiplier = 1.0
        if has_headings:
            if (not heading_seen) and (not lead_assigned) and (not infobox_like) and paragraph_sentences:
                multiplier = 1.5
                lead_assigned = True
            elif section_open_remaining > 0 and (not infobox_like):
                multiplier = 1.2
            elif infobox_like:
                multiplier = 0.5
        for s, norm in zip(paragraph_sentences, paragraph_seen_norms):
            seen.add(norm)
            sentences.append(s)
            structure_multipliers.append(multiplier)
            section_penalties.append(current_section_penalty)
        if section_open_remaining > 0 and paragraph_sentences:
            section_open_remaining -= 1
        if trailing_heading:
            section_open_remaining = 4
            current_section_penalty = _heading_section_penalty(trailing_heading_text)
    ...
    if not sentences:
        for sent in _split_sentences(text):
            ...
            seen.add(norm)
            sentences.append(s)
            structure_multipliers.append(1.0)
            section_penalties.append(1.0)
    ...
    for idx, sent in enumerate(sentences):
        ...
        kept_token_sets.append(toks)
    sentences, structure_multipliers = _dedupe_near_duplicate_sentences(sentences, structure_multipliers, threshold=0.60)
    section_penalties = [section_penalties[i] for i in kept_indices]
    ...
    for i, sent in enumerate(sentences):
        toks = _tokenize(sent)
        base = (sum(freq[t] for t in toks) / max(1, len(toks))) * math.log(1 + max(1, len(toks)))
        pos = 0.0
        if i < max(1, n_sent // 8):
            pos += _LEAD_BOOST
        if i > n_sent * 0.85:
            pos += _TAIL_BOOST
        score = base + _factual_signal(sent) + pos
        if i < len(structure_multipliers):
            score *= structure_multipliers[i]
        if i < len(section_penalties):
            score *= section_penalties[i]
        if len(toks) < _SHORT_TRIVIA_TOKEN_PENALTY_THRESHOLD:
            score *= _SHORT_TRIVIA_PENALTY
        scores.append(score)
...
def _render_extractive_summary(src_path: str, sentences: List[str]) -> str:
    ...
    lines = [
        f"# SUMM: {src_name}",
        "method: extractive-stdlib-tuned-v4",
        "",
        "## Extracted Sentences",
    ]
    if not sentences:
