...
_EDITORIAL_INTENT_VERB_RE = re.compile(
    r"\b(" + "|".join(_EDITORIAL_INTENT_VERBS) + r")\b",
    re.IGNORECASE,
)
_SCRATCH_OBJECT_REF_RE = re.compile(
    r"\b(response|comment|article|above|this|follow\s*up|followup)\b",
    re.IGNORECASE,
)
_SCRATCH_EVAL_SIGNAL_RE = re.compile(
    r"\b(fair|reasonable|accurate|critique|feedback|viable|viability|realistic|unrealistic|"
    r"unsustainable|fake|bullshit|nonsense|agree|disagree|summary)\b",
    re.IGNORECASE,
)


def is_scratch_context_eval_followup(*, user_text: str, locked_active: bool) -> bool:
    """Deterministic predicate for locked-scratch context-evaluation follow-ups."""
    if not bool(locked_active):
        return False
    ...
    # Allow longer follow-up prompts that include user draft/comment context.
    if token_count <= 0 or token_count > 140:
        return False
    has_question = "?" in raw
    low = raw.lower()
    first_person = bool(re.search(r"\b(my|i|i'm|i’d|i'd|me)\b", low))
    object_ref = bool(_SCRATCH_OBJECT_REF_RE.search(low))
    if has_question:
        return bool((first_person and has_question) or (object_ref and has_question))
    # Short evaluative follow-ups (including slang/profanity) should route to
    # meta-eval under lock even without a question mark.
    if token_count <= 24 and bool(_SCRATCH_EVAL_SIGNAL_RE.search(low)):
        return True
    return False


def has_editorial_instruction_intent(user_text: str) -> bool:
    """Canonical editorial-intent detector for scratch lane precedence."""
    raw = str(user_text or "")
    return bool(_EDITORIAL_INTENT_VERB_RE.search(raw))
...
def build_scratchpad_facts_block(
    *,
    ...
    locked = {int(i) for i in (locked_indices or set()) if int(i) > 0}
    editorial_intent_query = has_editorial_instruction_intent(q)
    context_eval_followup = is_scratch_context_eval_followup(
        user_text=q,
        locked_active=bool(locked),
    )
    for idx, rec in enumerate(records):
        ...
        if q_tokens:
            ...
            is_match = overlap > 0
            # Locked scratch mode is fail-closed by default.
            # Editorial/meta turns ("critique/review/edit/...") operate on the
            # locked object itself and may have weak lexical overlap, so keep
            # the locked record available in that case.
            if locked and overlap <= 0 and not (editorial_intent_query or context_eval_followup):
                continue
            if overlap <= 0:
