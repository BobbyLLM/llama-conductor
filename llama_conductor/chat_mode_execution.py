...
_FR_TOKEN_RE = re.compile(r"[A-Za-z0-9][A-Za-z0-9_\-]*")
_FR_TITLECASE_RE = re.compile(r"^[A-Z][a-z][A-Za-z0-9\-]*$")
_FR_ALLCAPS_RE = re.compile(r"^[A-Z]{2,}[A-Z0-9\-]*$")
_FR_ALNUM_ID_RE = re.compile(r"^(?=.*[A-Za-z])(?=.*\d)[A-Za-z0-9\-]+$")
_FR_DISMISSIVE_RE = re.compile(
    r"\b("
    r"who cares|don't care|do not care|whatever|shut up|"
    r"stop whining|cry more|skill issue|stupid question|"
    r"not worth it|waste of time"
    r")\b",
    re.IGNORECASE,
)
_FR_ENTITY_STOPWORDS = {
    "a", "an", "and", "are", "as", "at", "be", "but", "by", "for", "from", "has", "have", "he", "her",
    "his", "i", "if", "in", "is", "it", "its", "me", "my", "no", "not", "of", "on", "or", "our", "she",
    "so", "than", "that", "the", "their", "them", "there", "they", "this", "to", "we", "were", "what",
...
@lru_cache(maxsize=1)
def _fr_protected_phrase_map() -> Dict[str, Tuple[str, ...]]:
    """
    Build fixed protected phrase map:
    - seed with explicit phrase(s)
    - include idioms tagged feral_distress or safety
    """
    out: Dict[str, Tuple[str, ...]] = {
        "up shit creek": ("without a paddle", "in serious trouble"),
    }
    idioms_path = os.path.join(os.path.dirname(__file__), "cheatsheets", "idioms.jsonl")
    try:
        with open(idioms_path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if not ({"feral_distress", "safety"} & tags):
                    continue
                term = str(row.get("term") or "").strip().lower()
                if not term:
                    continue
                out.setdefault(term, tuple())
    except Exception:
        pass
    return out


def _extract_fr_key_entities(text: str) -> Set[str]:
    raw_tokens = [tok for tok in _FR_TOKEN_RE.findall(str(text or "")) if tok]
    entities: Set[str] = set()
    for tok in raw_tokens:
        low = tok.lower()
        if len(low) < 3:
            continue
        if low in _FR_ENTITY_STOPWORDS:
            continue
        keep = bool(
            _FR_TITLECASE_RE.match(tok)
            or _FR_ALLCAPS_RE.match(tok)
            or _FR_ALNUM_ID_RE.match(tok)
        )
        if keep:
            entities.add(tok)
    return entities
...
def _missing_key_entities(source_text: str, rewritten: str) -> List[str]:
    src_entities = sorted(_extract_fr_key_entities(source_text))
    if not src_entities:
        return []
    rew_tokens = set(_FR_TOKEN_RE.findall(str(rewritten or "")))
    missing = [tok for tok in src_entities if tok not in rew_tokens]
    return missing


def _idiom_garble_detected(base: str, rewritten: str) -> bool:
    base_norm = _norm_simple(base)
    rew_norm = _norm_simple(rewritten)
    if not base_norm or not rew_norm:
        return False
    for phrase, paraphrases in _fr_protected_phrase_map().items():
        p = _norm_simple(phrase)
        if not p:
            continue
        if p not in base_norm:
            continue
        if p in rew_norm:
            continue
        if any(_norm_simple(pp) and _norm_simple(pp) in rew_norm for pp in (paraphrases or ())):
            continue
        return True
    return False


def _direction_flip_detected(user_text: str, rewritten: str) -> bool:
    # Polarity floor only: if user asked a question, rewritten output must not go dismissive.
    if "?" not in str(user_text or ""):
        return False
    return bool(_FR_DISMISSIVE_RE.search(str(rewritten or "")))
...
def _guard_fun_rewrite(
    *,
    ...
    # FR semantic drift guard invariants (deterministic, fail-open telemetry).
    drift_reason = "none"
    missing_entities = _missing_key_entities(str(user_text or ""), rew)
    if missing_entities:
        drift_reason = "entity_drop"
    elif _idiom_garble_detected(base, rew):
        drift_reason = "idiom_garble"
    elif _direction_flip_detected(user_text, rew):
        drift_reason = "direction_flip"
    try:
        if state is not None:
            state.fr_semantic_drift_detected = bool(drift_reason != "none")
            state.fr_semantic_drift_reason = drift_reason
            state.fr_semantic_drift_entities_missing = list(missing_entities or [])
    except Exception:
        pass
...
async def maybe_handle_fun_fr_raw(
    *,
    ...
    codex_grounding_fn: Optional[Callable[[str], str]] = None,
) -> Optional[Any]:
    # Turn-local FUN execution diagnostics (consumed by router telemetry).
    try:
        state.fun_single_pass_enabled = False
        state.fun_model_calls_count = 0
    except Exception:
        pass

    vodka_runtime = getattr(state, "vodka", None) or no_op_vodka_cls()
    ctx_size = int(cfg_get("backend.llama_cpp.ctx_size", 8192) or 8192)
    ...
    if fun_mode == "fun":
        ...
        if (state_solver_answer or "").strip():
            ...
            if codex_grounding_fn is not None:
                text = codex_grounding_fn(text)
        ...
        if run_fun is None:
            ...
            # Item 15B / Checkpoint 1:
            # Remove pre-run_serious preview call from FUN non-deterministic path.
            # Single-pass collapse follow-on will continue in subsequent checkpoints.
            sel = await run_sync(select_fun_style_seed, state=state, user_text=user_text, base_text="")
            pool = list(sel.get("pool") or [])
            seed = str(sel.get("seed") or "")
            fun_diag = {"model_calls": 0}
            ...
            styled = (await run_sync(
                ...
                quote_pool=pool,
                seed_override=seed,
                base_answer_override="",
                constraints_block=constraints_block,
                single_pass=True,
                telemetry_sink=fun_diag,
                vodka=vodka_runtime,
                call_model=call_model_prompt,
                thinker_role="thinker",
            )).strip()
            try:
                state.fun_single_pass_enabled = True
                state.fun_model_calls_count = int(fun_diag.get("model_calls", 0))
            except Exception:
                pass
            ...
            if is_argumentative_prompt(user_text) and not is_argumentatively_complete(text):
                text = fallback_with_mode_header(text, "")
            text = _postguard(text)
            if codex_grounding_fn is not None:
                text = codex_grounding_fn(text)
    ...
    # serious_sticky or default_mode="serious" → run_serious pipeline
    if fun_mode == "serious":
        text = (await run_sync(
            run_serious,
            session_id=session_id,
    ...
    # default_mode="raw" → run_raw pipeline (when no sticky override is active)
    if fun_mode == "raw" and run_raw:
        text = (await run_sync(
            run_raw,
            session_id=session_id,
            user_text=user_text,
