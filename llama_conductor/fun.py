...
FUN_SINGLE_PASS_PROMPT = """You are answering in FUN mode using a seed quote.

You are given:
- CHAT HISTORY (for continuity)
- FACTS_BLOCK (grounding facts; may be empty)
- CONSTRAINTS_BLOCK (hard constraints; may be empty)
- QUESTION (current user query)
- SEED_QUOTE (tone anchor)

Rules:
- Produce one concise answer to QUESTION.
- Keep semantics grounded to FACTS_BLOCK when present.
- Obey CONSTRAINTS_BLOCK strictly when present.
- Match tone to SEED_QUOTE, but do not invent facts.
- Do not emit headers, labels, or analysis.
- Output only the answer body text.
"""
...
def run_fun(
    *,
    ...
    telemetry_sink: Optional[Dict[str, Any]] = None,
    vodka,
    call_model: Callable[..., str],
    ...
    if single_pass:
        _prompt = FUN_SINGLE_PASS_PROMPT
        single_prompt = (
            f"{_prompt}\n\n"
            f"CHAT HISTORY:\n{transcript or '[none]'}\n\n"
            f"FACTS_BLOCK:\n{facts_block or 'NONE'}\n\n"
            f"CONSTRAINTS_BLOCK:\n{constraints_block or 'NONE'}\n\n"
            f"SEED_QUOTE:\n{seed}\n\n"
            f"QUESTION:\n{user_text}\n\n"
            "ANSWER:\n"
        )
        if isinstance(telemetry_sink, dict):
            telemetry_sink["model_calls"] = int(telemetry_sink.get("model_calls", 0)) + 1
        body = call_model(
            role=thinker_role,
