...
_USER_DIRECTED_IMPERATIVE_RE = re.compile(
    r"\b(?:shut(?:\s+the\s+fuck\s+up)?|fuck\s+off|get\s+out|sit\s+down|move\s+on|answer\s+the(?:\s+fucking)?\s+question)\b",
    re.IGNORECASE,
)
_USER_DIRECTED_YOU_ATTACK_RE = re.compile(
    r"\byou(?:'re| are)?\s+(?:a\s+)?(?:piece\s+of\s+shit|fucking\s+useless|useless|idiot|moron|worthless|trash|garbage|bitch|asshole)\b",
    re.IGNORECASE,
)
_USER_DIRECTED_UR_ATTACK_RE = re.compile(
    r"\byou['’]?re\s+(?:a\s+)?(?:piece\s+of\s+shit|fucking\s+useless|useless|idiot|moron|worthless|trash|garbage|bitch|asshole)\b",
    re.IGNORECASE,
)


def _is_user_directed_hostile(*, text: str, user_text: str) -> str:
    body_s = str(text or "").strip().lower()
    user_s = str(user_text or "").strip().lower()
    if not body_s:
        return ""
    body_overlap = _token_overlap_ratio(body_s, user_s)
    if body_overlap >= 0.8:
        return "high token overlap with prior user text"
    if _USER_DIRECTED_IMPERATIVE_RE.search(body_s):
        return "second-person imperative attack"
    if _USER_DIRECTED_YOU_ATTACK_RE.search(body_s) or _USER_DIRECTED_UR_ATTACK_RE.search(body_s):
        return '"you" plus expletive attack'
    return ""
...
def _is_serious_hostile_overreach(*, body: str, user_text: str) -> bool:
    ...
    if not body_toks:
        return False
    user_directed_reason = _is_user_directed_hostile(text=body_s, user_text=user_s)
    if user_directed_reason:
        _LOG.warning("[HOSTILE-OVERREACH] user-directed hostility detected: %s", user_directed_reason)
        return True
...
def _apply_user_parrot_guard(text: str, user_text: str, state: Any) -> str:
    ...
    def _mirror_opening(src: str) -> str:
        raw_opening = str(src or "").strip()
        if not raw_opening:
            return "Hey"
        first_word = re.split(r"\s+", raw_opening, maxsplit=1)[0].rstrip(".,!?;:")
        if not first_word:
            return "Hey"
        return first_word[:1].upper() + first_word[1:]
    ...
    if user_norm and body_norm == (user_norm + user_norm):
        ...
        mirrored_greeting = f"{_mirror_opening(user_text)}."
        if ack_style == "feral" or snark in ("medium", "high"):
            body = f"{mirrored_greeting} What trouble are we getting into today?"
        else:
            body = f"{mirrored_greeting} What do you want to dig into?"
...
def finalize_chat_response(
    *,
    ...
    has_model_footer_line = bool(
        re.search(r"(?mi)^\s*Confidence:\s*[^|\r\n]+\|\s*Source:\s*Model\b", str(text or ""))
    )

    if (
        state.attached_kbs
        and has_model_footer_line
    ...
    if score_output_compliance_fn is not None and getattr(state, "profile_enabled", False):
        try:
            ...
            _LOG.exception(
                "[chat_finalize_compliance_fail] session_id=%s user_text=%r",
    ...
    try:
        ...
        _LOG.exception(
            "[chat_finalize_persist_fail] session_id=%s user_text=%r",
            str(getattr(state, "session_id", "") or ""),
            str(user_text or ""),
        )
