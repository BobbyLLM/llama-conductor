...
_ROUTER_LOG_PATH = Path(__file__).resolve().parents[1] / "launch-logs" / "router.log"
try:
    _ROUTER_LOG_PATH.parent.mkdir(parents=True, exist_ok=True)
    _root_logger = logging.getLogger()
    if not any(
        isinstance(h, logging.FileHandler) and getattr(h, "baseFilename", "") == str(_ROUTER_LOG_PATH)
        for h in _root_logger.handlers
    ):
        _router_file_handler = logging.FileHandler(_ROUTER_LOG_PATH, mode="a", encoding="utf-8")
        _router_file_handler.setLevel(logging.WARNING)
        _root_logger.addHandler(_router_file_handler)
except Exception:
    pass

_router_diag_logger = logging.getLogger("moa.router.diag")

def _soft_alias_command_with_nl_search(text: str, state: SessionState) -> Optional[str]:
    alias_cmd = _chat_soft_alias_command(
        text=text,
        state=state,
    ...
    if not raw or _is_command(raw):
        return None
    if not is_bare_nl_search_alias(raw):
        return None
    query = resolve_nl_search_query(state)
    if not query:
        return None
    return f">>web {query}"
...
from .model_calls import MODEL_REQUEST_TRACE, call_model_prompt, call_model_messages
...
# Required modules
from .vodka_filter import Filter as VodkaFilter, purge_session_memory_jsonl, purge_ctc_compaction_jsonl, purge_vodka_ctx_facts
from .serious import run_serious
print(f"[vodka_module_path] {VodkaFilter.__module__}:{getattr(VodkaFilter, '__file__', 'n/a')}", flush=True)
...
try:
    ...
    EncoderServer = None  # type: ignore
...
from .nl_search_alias import is_bare_nl_search_alias, resolve_nl_search_query, summarize_nl_search_alias_response
...
try:
    from .kaioken_vnext import (
        KaiokenControlObject,
        classify_turn as _kaioken_vnext_classify,
        classify_fast_pass as _kaioken_vnext_fast_pass,
        log_kco_telemetry as _kaioken_vnext_log_telemetry,
        assert_kaioken_mode_exclusion,
    )
    from .kaioken_asset_loader import load_kaioken_assets, select_phrases
    _KAIOKEN_VNEXT_AVAILABLE = True
except Exception as _knext_err:
    _KAIOKEN_VNEXT_AVAILABLE = False
    print(f"[kaioken_vnext] import failed: {_knext_err}", flush=True)
from .w2a_session_continuity import run_w2a
try:
...
_CODEX_TARGETED_FACTUAL_LOOKUP_RE = re.compile(
    r"^\s*(?:what\s+year|what\s+date|when|who|where|which|what\s+is|what\s+are|what\s+does|define|tell\s+me\s+about|how\s+many|how\s+much)\b",
    re.IGNORECASE,
)
...
_CODEX_SUMM_SIGNAL_STOPWORDS = {
    "the", "and", "for", "with", "from", "that", "this", "what", "why", "how",
    "does", "did", "are", "was", "were", "can", "could", "would", "might",
    "should", "shall", "will", "may", "must", "have", "has", "had", "in", "of",
    "to", "a", "an", "it", "is", "as", "by", "on", "at", "or", "but", "been",
    "than", "then", "there", "their", "they", "them", "this", "these", "those",
}
_CODEX_MECHANISTIC_SUMM_MARKERS = {
    "prediction",
    "predictions",
    "threat",
    "danger",
...
def _codex_signal_tokens(text: str) -> List[str]:
    return [
        t
        for t in _codex_norm_tokens(text)
        if t not in _CODEX_SUMM_SIGNAL_STOPWORDS
    ]


def _codex_summary_has_mechanistic_markers(text: str) -> bool:
    return bool(set(_codex_signal_tokens(text)).intersection(_CODEX_MECHANISTIC_SUMM_MARKERS))
...
def _codex_turn_shape(text: str) -> str:
    try:
        shape = classify_question_shape(str(text or ""))
        return str(getattr(shape, "kind", "") or "").strip().lower()
    except Exception:
        return ""
...
def _codex_is_lookupish(query: str) -> bool:
    ...
    if _CONVERSATIONAL_CLARIFICATION_RE.search(raw):
        return False
    return bool(_CODEX_LOOKUPISH_RE.search(raw))


def _codex_is_targeted_factual_lookup(query: str) -> bool:
    raw = str(query or "").strip()
    if not raw:
        return False
    if re.search(r"\b(?:could|would|might|should|shall|will|may|must)\b", raw, flags=re.IGNORECASE):
        return False
    return bool(_CODEX_TARGETED_FACTUAL_LOOKUP_RE.search(raw))
...
def _sidecar_codex_query(query: str, *, state: SessionState) -> Tuple[bool, Dict[str, Any], str]:
    ...
    try:
        ...
        if not q_toks:
            return False, {"state": "healthy_miss"}, ""
        q_signal_toks = set(_codex_signal_tokens(q))
        q_shape_kind = _codex_turn_shape(q)
        if q_shape_kind in {"speculative", "unknown"}:
            return False, {"state": "healthy_miss"}, ""
        if q_shape_kind == "factual" and not _codex_is_targeted_factual_lookup(q):
            return False, {"state": "healthy_miss"}, ""

        def _summ_candidates_for_page(stem: str) -> List[Path]:
            stems = []
            for variant in (
                stem,
                stem.replace("-", "_"),
            ...
            for variant in stems:
                candidates = list(summ_dir.glob(f"**/SUMM_{variant}*.md"))
                if not candidates:
                    candidates = list(summ_dir.glob(f"**/SUMM_*{variant}*.md"))
                if candidates:
                    break
        ...
        q_signal_size = len(q_signal_toks)
        def _allow_single_signal_overlap(score: int) -> bool:
            if score <= 0:
                return False
            # One-token overlaps are too broad for generic definitional lookups
            # like "What is a blitter?" but remain useful for multi-signal turns
            # such as the pain-science and Commodore probes.
            return score >= 2 or q_signal_size > 1
        for idx, e in enumerate(entries):
            ...
            if score <= 0 and summ_dir.exists():
                rel = str(e.get("rel", "") or "").strip().replace("\\", "/")
                page_stem = os.path.splitext(os.path.basename(rel))[0]
                summ_candidates = _summ_candidates_for_page(page_stem)
                if summ_candidates:
                    try:
                        summ_text = summ_candidates[0].read_text(encoding="utf-8")
                    except Exception:
                        summ_text = ""
                    if summ_text:
                        summ_signal_toks = set(_codex_signal_tokens(summ_text))
                        if summ_signal_toks:
                            summ_overlap = int(len(q_signal_toks.intersection(summ_signal_toks)))
                            if q_shape_kind == "mechanistic":
                                if summ_overlap >= 1 and _codex_summary_has_mechanistic_markers(summ_text):
                                    score = summ_overlap
                            elif _allow_single_signal_overlap(summ_overlap):
                                score = summ_overlap
            if _allow_single_signal_overlap(score):
        ...
        for rel in pages:
            page_stem = os.path.splitext(os.path.basename(rel))[0]
            summ_candidates = _summ_candidates_for_page(page_stem) if summ_dir.exists() else []
            if summ_candidates:
                try:
...
def _has_ctc_compaction_snapshot(messages: List[Dict[str, Any]]) -> bool:
    for msg in messages or []:
        if str(msg.get("role") or "").strip().lower() != "system":
            continue
        content = str(msg.get("content") or "").strip()
        if content.startswith("[CTC_COMPACTION]"):
            return True
    return False


def _looks_like_ctc_session_recall(user_text: str, *, vodka: Any = None) -> bool:
    q = " ".join(str(user_text or "").strip().lower().split())
    if not q:
        return False
    # Reuse Vodka's existing recall heuristics when available.
    if vodka is not None:
        for meth_name in ("_is_recall_query", "_looks_like_prior_session_reference"):
            meth = getattr(vodka, meth_name, None)
            if callable(meth):
                try:
                    if bool(meth(q)):
                        return True
    ...
    if any(h in q for h in hints):
        return True
    if re.search(r"\b(?:what|which|who|where|when|how)\b.*\b(?:must\s+stay\s+out\s+of\s+scope|should\s+stay\s+out\s+of\s+scope|stay\s+out\s+of\s+scope|not\s+bring\s+back|must\s+not|should\s+not)\b", q):
        return True
    if re.search(r"\b(?:what|who)\s+(?:was|is)\s+(?:the\s+)?(?:name|ship|captain|first line|opening line)\b", q):
        return True
    return False


def _looks_like_ctc_external_lookup_query(user_text: str) -> bool:
    q = " ".join(str(user_text or "").strip().lower().split())
    if not q:
        return False
    hints = (
    ...
    if any(h in q for h in hints):
        return True
    if re.search(r"\b(search|lookup|look\s+up|find)\b", q):
        return True
    return False


def _looks_like_ctc_ingestion_turn(user_text: str) -> bool:
    """True for smoke/script payload turns that should bypass recall lookup."""
    q = str(user_text or "").lstrip()
    return bool(
        q.startswith("[SESSION-CHUNK-")
        or q.startswith("[SMOKE-CHUNK-")
    )
...
def _emit_kaioken_telemetry(
    *,
    ...
    try:
        ...
        kco = getattr(state, "kco", None)
        if kco is not None:
            advisor_outputs = getattr(kco, "advisor_outputs", None)
            if advisor_outputs:
                outcome = dict(outcome or {})
                outcome["advisor_outputs"] = advisor_outputs
...
def _apply_kaioken_vnext_advisory(
    *,
    state: SessionState,
    user_text: str,
    prior_turn_text: str = "",
    prior_assistant_text: str = "",
    prior_macro_register: Optional[str] = None,
) -> Optional[KaiokenControlObject]:
    if not _KAIOKEN_VNEXT_AVAILABLE:
        state.kco = None
        return None
    mode = str(getattr(state, "kaioken_mode", "") or "").strip().lower()
    if mode != "vnext":
        state.kco = None
        return None
    # W3 fast pass - macro only, fires first to unblock W4
    # Advisory only at this stage - W4 wiring comes in Phase 5
    try:
        _fast_pass_macro = _kaioken_vnext_fast_pass(str(user_text or ""))
        print(f"[kaioken_vnext] fast_pass macro={_fast_pass_macro}", flush=True)
    except Exception as _fp_err:
        _fast_pass_macro = "working"
        print(f"[kaioken_vnext] fast_pass failed: {_fp_err}", flush=True)
    try:
        kco = _kaioken_vnext_classify(
            str(user_text or ""),
            prior_turn_text=str(prior_turn_text or ""),
            prior_assistant_text=str(prior_assistant_text or ""),
            prior_macro_register=prior_macro_register,
        )
        _kaioken_vnext_log_telemetry(kco, turn_index=int(getattr(state, "turn_index", 0) or 0))
        state.kco = kco
        print(
            f"[kaioken_vnext] full_pass macro={kco.macro_register}"
            f" refusal={kco.refusal}"
            f" risk={kco.risk_level}"
            f" empathy_override={kco.empathy_override}"
            f" profile_floor={kco.profile_floor}"
            f" confidence_floor={kco.confidence_floor}",
            flush=True,
        )
        return kco
    except Exception as _kco_err:
        print(f"[kaioken_vnext] classify_turn failed: {_kco_err}", flush=True)
        state.kco = None
        return None
...
def _build_kaioken_soft_constraints(*, state: SessionState, user_text: str) -> str:
    ...
    if "vulnerable_under_humour" in effective_subs:
        ...
        hints.append("Respond to this turn only. Do not carry prior topics forward unless the user explicitly continues them. Match the user's register â€” banter gets banter, short gets short. No task-forward language.")
    if literal_followup:
        hints.extend([
...
_KAIOKEN_PEP_TALK_RE = re.compile(
    r"\b("
    r"you(?:'|â€™)re not|you are not|you got this|keep going|keep pushing|keep debugging|push through|start small|"
    r"focus on|many developers|age (?:isn(?:'|â€™)t|is not)|"
    r"progress over perfection|you(?:'|â€™)re human|you are human|"
...
_KAIOKEN_ADVICE_RE = re.compile(
    ...
    r"do this|next step|keep coding|ship what you have|let(?:'|â€™)s\b|lets\b"
    r")\b",
    re.IGNORECASE,
)
_KAIOKEN_REASSURE_RE = re.compile(
    r"\b("
    r"you(?:'|â€™)re not|you are not|it(?:'|â€™)s okay|it is okay|"
    r"you(?:'|â€™)ll be fine|you will be fine|you(?:'|â€™)ve got this|you got this|"
    r"not a flaw in you|not a sign (?:you(?:'|â€™)re|you are) failing|not failing"
    r")\b",
    re.IGNORECASE,
)
...
_KAIOKEN_JOKE_EXPLAIN_RE = re.compile(
    r"\b("
    r"you(?:'|â€™)re comparing|underneath that joke|using humor to mask|"
    r"the punchline|you(?:'|â€™)re using humor|joke is that|what you really mean"
    r")\b",
    re.IGNORECASE,
)
...
_KAIOKEN_OWNERSHIP_DESCRIPTOR_RE = re.compile(
    r"\b("
    r"(?:you(?:(?:'|â€™)re| are)\s+(?:\w+\s+){0,3}?(?:unstable|broken|worthless|damaged|weak|fraud(?:ulent)?))|"
...
_KAIOKEN_DIAGNOSTIC_RE = re.compile(
    ...
    r"you(?:'|â€™)re still treating|you are still treating|you need to|the problem is|"
...
_RETRIEVAL_FOLLOWUP_DEICTIC_RE = re.compile(
    r"\b("
    r"explain that(?: then)?|"
...
def _is_short_deictic_retrieval_followup(user_text: str) -> bool:
    ...
    if token_count > 12:
        return False
    low = raw.lower()
    if _RETRIEVAL_FOLLOWUP_DEICTIC_RE.search(low):
        return True
...
def _build_retrieval_followup_anchor_constraints(*, user_text: str, prior_assistant_text: str) -> str:
    if not _is_short_deictic_retrieval_followup(user_text):
        return ""
    prior_source = _prior_assistant_retrieval_source_label(prior_assistant_text)
    if not prior_source:
        return ""
    src_label = "WEB" if prior_source == "web" else ("WIKI" if prior_source == "wiki" else "retrieved")
    return "\n".join(
        [
            ...
            f"- Anchor the answer to the injected {src_label} evidence block already in context.",
...
_QUOTE_SPAN_RE = re.compile(r"[\"â€œâ€'â€˜â€™]([^\"â€œâ€'â€˜â€™]{3,180})[\"â€œâ€'â€˜â€™]")
...
def _finalize_chat_response(
    *,
    ...
    try:
        state.last_turn_web_evidence_injected = bool(getattr(state, "turn_web_evidence_injected", False))
    except Exception:
        pass
...
async def _startup_runtime() -> None:
    try:
        ...
        compaction_purged = purge_ctc_compaction_jsonl(base, vodka_subdir=subdir)
        _dbg(f"[DEBUG] startup ctc-compaction purge deleted={compaction_purged}")
    ...
    try:
        ...
        pass
    try:
        if _KAIOKEN_VNEXT_AVAILABLE:
            _vnext_mode = str(cfg_get("kaioken.mode", "") or "").strip().lower()
            _vnext_legacy = bool(cfg_get("kaioken.legacy_mode_enabled", False))
            _vnext_serious = str(cfg_get("kaioken.base_mode", "") or "").strip().lower() == "serious"
            assert_kaioken_mode_exclusion(
                legacy_enabled=_vnext_legacy,
                serious_enabled=_vnext_serious,
                vnext_enabled=(_vnext_mode == "vnext"),
            )
            print(f"[kaioken_vnext] startup check passed mode={_vnext_mode} legacy={_vnext_legacy}", flush=True)
            print(
                f"[kaioken_vnext] startup legacy_mode_enabled={_vnext_legacy} base_mode={cfg_get('kaioken.base_mode', 'unset')}",
                flush=True,
            )
            print(
                f"[kaioken_vnext] startup available=True mode={cfg_get('kaioken.mode', 'unset')}",
                flush=True,
            )
    except RuntimeError as _knext_exc:
        raise RuntimeError(f"kaioken_vnext startup exclusion: {_knext_exc}")
    except Exception as _knext_exc:
        print(f"[kaioken_vnext] startup check error: {_knext_exc}", flush=True)
...
async def v1_chat_completions(req: Request):
    ...
    try:
        ...
        MODEL_REQUEST_TRACE.set(
            {
                "session_id": session_id,
                "user_agent": ua,
                "client_family": (
                    "firefox" if "firefox" in ua.lower()
                    else ("gptmobile" if "mobile" in ua.lower() else "unknown")
                ),
                "request_path": str(req.url.path or "").strip(),
            }
        )
    except Exception:
        pass
    try:
        state.prior_turn_web_evidence_injected = bool(getattr(state, "last_turn_web_evidence_injected", False))
        state.last_turn_web_evidence_injected = False
        state.turn_web_evidence_injected = False
    except Exception:
        pass
    ...
    # Apply flush boundary â€” exclude pre-flush history
    _flush_idx = getattr(state, "last_flush_message_index", 0)
    if _flush_idx > 0 and _flush_idx < len(raw_messages):
        raw_messages = raw_messages[_flush_idx:]
    _emit_image_trace(
        "chat_ingress",
        {
            "session_id": session_id,
    ...
    preflight_handled, preflight_resp, selector, user_text, user_text_raw, raw_messages, sensitive_override_once = _handle_preflight(
        ...
        soft_alias_command=_soft_alias_command_with_nl_search,
    ...
    try:
        live_ctx_size = int(getattr(state, "ctx_size", 0) or int(cfg_get("backend.llama_cpp.ctx_size", 8192) or 8192))
    except Exception:
        live_ctx_size = int(cfg_get("backend.llama_cpp.ctx_size", 8192) or 8192)
    try:
        live_prompt_chars = sum(
            len(str(m.get("content", "") or ""))
            for m in (raw_messages or [])
            if str(m.get("role") or "").lower() in ("user", "assistant", "system")
        )
        live_pressure_pct = round(
            min(1.0, live_prompt_chars / max(1, int(live_ctx_size or 1))) * 100.0,
            2,
        )
        state.ctc_current_prompt_chars = int(live_prompt_chars)
        state.ctc_current_prompt_pressure_pct = float(live_pressure_pct)
        state.ctc_current_prompt_ctx_size = int(live_ctx_size or 0)
        state.ctc_current_prompt_turn = int(getattr(state, "profile_turn_counter", 0) or 0)
    except Exception:
        pass

    compaction_status_payload: Dict[str, Any] = {}
    compaction_ingestion_turn = bool(_looks_like_ctc_ingestion_turn(user_text))
    compaction_snapshot_active = bool(_has_ctc_compaction_snapshot(raw_messages))
    if not compaction_snapshot_active and state.vodka is not None and hasattr(state.vodka, "get_ctc_compaction_status"):
        try:
            compaction_status_payload = state.vodka.get_ctc_compaction_status(
                session_id,
                user_turn_hint=int(getattr(state, "profile_turn_counter", 0) or 0),
            ) or {}
            compaction_snapshot_active = bool(
                int(compaction_status_payload.get("record_count", 0) or 0) > 0
                and str(compaction_status_payload.get("last_snapshot_id", "") or "").strip()
            )
        except Exception:
            compaction_status_payload = {}
    compaction_default_session_recall = bool(cfg_get("compaction.compaction_default_session_recall", True))
    compaction_external_lookup_intent = _looks_like_ctc_external_lookup_query(user_text)
    compaction_definitional_intent = bool(
        _cs_strict_lookup_re.match(
            str(user_text or "").strip()
        )
    )
    compaction_recall_intent = _looks_like_ctc_session_recall(user_text, vodka=state.vodka)
    compaction_snapshot_first_mode = bool(
        compaction_snapshot_active
        and (not compaction_ingestion_turn)
        and (not compaction_external_lookup_intent)
        and (not compaction_definitional_intent)
        and (compaction_default_session_recall or compaction_recall_intent)
        and any(m.get("role") == "assistant" for m in raw_messages)
    )
    compaction_deterministic_answer = ""
    compaction_deterministic_stage = ""
    compaction_debug_meta: Dict[str, Any] = {
        "gate_active": bool(compaction_snapshot_first_mode),
        "snapshot_first_mode": bool(compaction_snapshot_first_mode),
        "snapshot_active": bool(compaction_snapshot_active),
        ...
        "lookup_reason": "",
        "lookup_preview": "",
        "snapshot_record_count": int(compaction_status_payload.get("record_count", 0) or 0),
        "snapshot_last_id": str(compaction_status_payload.get("last_snapshot_id", "") or ""),
        "session_recall_intent": bool(compaction_recall_intent),
        "external_lookup_intent": bool(compaction_external_lookup_intent),
        "ingestion_turn": bool(compaction_ingestion_turn),
        "default_session_recall": bool(compaction_default_session_recall),
    }
    if compaction_snapshot_first_mode:
        # Snapshot-first routing: session recall should stay on the compaction boundary.
        # A miss stays in-session and should not reopen external retrieval lanes.
        setattr(state, "turn_retrieval_track", "C")
        setattr(state, "turn_cheatsheet_hit", False)
        setattr(state, "turn_web_retrieval_verified", False)
        setattr(state, "evidence_context_wiki_fallback_active", False)
        if state.vodka is not None and hasattr(state.vodka, "resolve_ctc_compaction_query"):
            try:
                compaction_debug_meta["lookup_attempted"] = True
                compaction_deterministic_answer, compaction_meta = state.vodka.resolve_ctc_compaction_query(
                    session_id,
                    user_text,
                )
                compaction_debug_meta.update(
                    {
                        "snapshot_active": bool(compaction_meta.get("snapshot_found", False)),
                        "lookup_found": bool(compaction_deterministic_answer),
                        "lookup_source": str(compaction_meta.get("source", "") or ""),
                        "lookup_field": str(compaction_meta.get("field", "") or ""),
                        "lookup_reason": str(compaction_meta.get("reason", "") or ""),
                        "lookup_preview": safe_preview(compaction_deterministic_answer, max_len=220),
                        "snapshot_record_count": int(compaction_meta.get("record_count", 0) or 0),
                        "snapshot_last_id": str(compaction_meta.get("last_snapshot_id", "") or ""),
                    }
                )
                if compaction_deterministic_answer:
                    compaction_deterministic_stage = "ctc_compaction_snapshot_lookup"
                    state.turn_footer_source_override = "CTC Compaction"
                    state.turn_footer_confidence_override = "high"
                    state.turn_source_url_override = ""
                    state.turn_footer_source_override_snapshot = (
                        f"{compaction_meta.get('source', '')}:{compaction_meta.get('field', '')}".strip(":")
                    )
                elif compaction_snapshot_first_mode and not compaction_external_lookup_intent:
                    compaction_deterministic_answer = ""
                    compaction_deterministic_stage = "ctc_compaction_session_miss"
                    compaction_debug_meta["lookup_found"] = True
                    compaction_debug_meta["lookup_source"] = "session_miss"
                    compaction_debug_meta["lookup_field"] = "session_recall"
                    compaction_debug_meta["lookup_reason"] = "not_in_session_record"
                    compaction_debug_meta["lookup_preview"] = "not in session record"
                    state.turn_footer_source_override = "CTC Compaction"
                    state.turn_footer_confidence_override = "unverified"
                    state.turn_source_url_override = ""
                    state.turn_footer_source_override_snapshot = "session_miss"
            except Exception:
                compaction_debug_meta["lookup_attempted"] = True
                compaction_deterministic_answer = ""
                compaction_deterministic_stage = ""
    setattr(state, "turn_compaction_debug", compaction_debug_meta)
    compaction_debug_line = (
        "[ctc_compaction_debug] "
        f"ingestion={int(bool(compaction_debug_meta.get('ingestion_turn')))} "
        f"snapshot_first={int(bool(compaction_debug_meta.get('snapshot_first_mode')))} "
        f"gate={int(bool(compaction_debug_meta.get('gate_active')))} "
        f"snapshot={int(bool(compaction_debug_meta.get('snapshot_active')))} "
        f"attempted={int(bool(compaction_debug_meta.get('lookup_attempted')))} "
        f"found={int(bool(compaction_debug_meta.get('lookup_found')))} "
        f"source={compaction_debug_meta.get('lookup_source', '')} "
        f"field={compaction_debug_meta.get('lookup_field', '')} "
        f"reason={compaction_debug_meta.get('lookup_reason', '')} "
        f"return={compaction_debug_meta.get('lookup_preview', '')} "
        f"record_count={compaction_debug_meta.get('snapshot_record_count', 0)} "
        f"snapshot_id={compaction_debug_meta.get('snapshot_last_id', '')} "
        f"default_session_recall={int(bool(compaction_debug_meta.get('default_session_recall')))} "
        f"external_lookup={int(bool(compaction_debug_meta.get('external_lookup_intent')))}"
    )
    logging.warning(compaction_debug_line)
    _dbg(compaction_debug_line)
    ...
    turn_question_shape = classify_question_shape(str(user_text or ""))
    turn_question_shape_is_soft = bool(
        turn_question_shape.kind in {"analogical", "speculative", "mechanistic"}
        and str(turn_question_shape.confidence or "").strip().lower() == "high"
    )
    # Policy for Problem 3:
    # - factual / unknown / low-confidence shapes keep the existing hard gate
    # - only high-confidence analogical/speculative/mechanistic turns bypass the
    #   cheatsheets deterministic short-circuit so Codex content remains in
    #   the turn for synthesis
    try:
        if editorial_instruction_anchor_active:
            turn_macro = "working"
            turn_subsignals = set()
    ...
    kco = _apply_kaioken_vnext_advisory(
        state=state,
        user_text=str(user_text or ""),
        prior_turn_text=str(getattr(state, "last_user_text", "") or ""),
        prior_assistant_text=str(getattr(state, "last_assistant_text", "") or ""),
        prior_macro_register=prior_macro_norm or None,
    )
    if kco is not None:
        run_w2a(
            current_turn=str(user_text or ""),
            recent_user_turns=list(getattr(state, "kaioken_recent_user_turns", []) or []),
            kco=kco,
        )
        _kaioken_vnext_log_telemetry(kco, turn_index=int(getattr(state, "turn_index", 0) or 0))
    try:
    ...
    codex_err = ""
    compaction_retrieval_gate_active = bool(compaction_snapshot_first_mode)
    if not compaction_retrieval_gate_active:
        try:
        ...
        codex_payload = {"state": "compaction_gate"}
    ...
    if compaction_retrieval_gate_active:
        facts_block = ""
    elif (
    ...
    state.turn_source_url_override = ""
    state.turn_retrieval_track = "C" if compaction_retrieval_gate_active else ""
    ...
    try:
        if compaction_retrieval_gate_active:
            # Keep the prompt on the boundary snapshot instead of falling through to
            # external retrieval lanes for session-recall questions.
            pass
        elif (_resolve_cheatsheets_turn is not None) and _CHEATSHEETS_RUNTIME_ENABLED and (not scratch_lane_locked):
            macro = turn_macro
            subsignals = set(turn_subsignals)
            wiki_lookup_fn = None if str(turn_question_shape.kind or "").strip().lower() == "speculative" else _sidecar_wiki_query
            if bool(getattr(state, "prior_turn_web_evidence_injected", False)) and _is_short_deictic_retrieval_followup(str(user_text or "")):
                wiki_lookup_fn = None
            ...
            cheat = _resolve_cheatsheets_turn(
                ...
                state=state,
                encoder=EncoderServer.get_encoder() if EncoderServer is not None else None,
                question_shape=turn_question_shape,
            )
            ...
            cheat_associative_hint = str(getattr(cheat, "associative_hint", "") or "").strip()
            state.turn_cheatsheet_hit = bool(
                ...
                and (
                    ...
                    or bool(cheat_associative_hint)
                )
            )
            ...
            if cheat_associative_hint:
                facts_block = (facts_block or "") + (
                    f"\n\n[Also resonates: {cheat_associative_hint}]"
                )
    except Exception:
        pass

    cheatsheets_resolved = bool(
        state.turn_cheatsheet_hit
        or cheatsheets_deterministic_answer
        or facts_block
    )
    ...
    codex_text = ""
    if codex_ok and not compaction_retrieval_gate_active:
        ...
        if codex_text and (not lock_active_now) and not _CONVERSATIONAL_CLARIFICATION_RE.search(user_text):
            codex_strict_lookup = bool((codex_payload or {}).get("strict", False))
            soft_codex_grounding_turn = bool(
                turn_question_shape_is_soft
                and str(getattr(turn_question_shape, "kind", "") or "").strip().lower()
                in {"analogical", "speculative", "mechanistic"}
                and not codex_strict_lookup
            )
            blended_grounding_active = bool(
                turn_question_shape_is_soft
                and bool(state.turn_cheatsheet_hit)
                and bool((facts_block or "").strip())
                and not codex_strict_lookup
            )
            if blended_grounding_active:
                blended_codex_instruction = (
                    "[Use the facts above as grounding. Synthesise across all sources provided.]"
                )
                ...
                state.turn_footer_source_override = "Mixed"
                state.turn_footer_confidence_override = "medium"
                state.turn_source_url_override = ""
                cheatsheets_deterministic_answer = ""
                codex_deterministic_answer = ""
            elif cheatsheets_deterministic_answer:
                facts_block = (
                    f"{facts_block}\n\n{codex_text}".strip()
                    if facts_block
                    else codex_text
                ...
                grounding_instruction = (
                    "[Use the facts above as grounding. Synthesise across all sources provided.]"
                    if soft_codex_grounding_turn
                    else "[Answer using only the facts above. Do not use general knowledge.]"
                )
                _codex_grounded_text = f"{codex_text}\n\n{grounding_instruction}"
                facts_block = (
                    f"{facts_block}\n\n{_codex_grounded_text}".strip()
    ...
    if editorial_instruction_anchor_active:
        ...
        state.post_editorial_context_inject_pending = False
    retrieval_followup_anchor = _build_retrieval_followup_anchor_constraints(
        user_text=str(user_text or ""),
        prior_assistant_text=str(getattr(state, "last_assistant_text", "") or "").strip() or _last_assistant_text(history_text_only),
    )
    if retrieval_followup_anchor:
        constraints_block = (
            f"{constraints_block}\n\n{retrieval_followup_anchor}".strip()
            if constraints_block
            else retrieval_followup_anchor
        )
    ...
    if bool(getattr(state, "evidence_context_wiki_fallback_active", False)) and not scratch_lock_active and not codex_has_healthy_hit and not _CONVERSATIONAL_CLARIFICATION_RE.search(user_text):
    ...
    deterministic_stage = ""
    if compaction_deterministic_answer:
        deterministic_answer = compaction_deterministic_answer
        deterministic_stage = compaction_deterministic_stage
    elif cheatsheets_deterministic_answer:
    ...
    if deterministic_answer and (not lock_active):
        try:
            if cheatsheets_deterministic_answer and (not turn_question_shape_is_soft):
                if retrieval_guard_intent_now == "quote_source":
    ...
    if codex_text:
        def _codex_grounding_fn(
            t: str,
            _src=str((codex_payload or {}).get("raw_text", codex_text) or codex_text),
            _usr=str(user_text or ""),
            _strict=_codex_strict,
            _shape=turn_question_shape,
        ) -> str:
            _, validated = codex_validate_answer(t, _src, _usr, strict=_strict, question_shape=_shape)
            return validated
    ...
    _emit_kaioken_telemetry(
        ...
        outcome={
            ...
            "ctc_compaction_debug": dict(getattr(state, "turn_compaction_debug", {}) or {}),
        },
    )
