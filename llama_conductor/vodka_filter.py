...
CTC_COMPACTION_PREFIX = "[CTC_COMPACTION] "
COMPACTION_PRESSURE_MIN_PCT = 70.0
COMPACTION_PRESSURE_MAX_PCT = 90.0
COMPACTION_PRESSURE_DEFAULT_PCT = 70.0
_LOG = logging.getLogger(__name__)
COMPACTION_PRESSURE_RAW = cfg_get("compaction.compaction_pressure_pct", COMPACTION_PRESSURE_DEFAULT_PCT)


def _load_compaction_pressure_pct() -> float:
    raw = COMPACTION_PRESSURE_RAW
    try:
        pct = float(raw)
    except Exception:
        pct = COMPACTION_PRESSURE_DEFAULT_PCT
        _LOG.warning(
            "compaction.compaction_pressure_pct=%r invalid; using %g",
            raw,
            COMPACTION_PRESSURE_DEFAULT_PCT,
        )
        return pct

    clamped = max(COMPACTION_PRESSURE_MIN_PCT, min(COMPACTION_PRESSURE_MAX_PCT, pct))
    if clamped != pct:
        _LOG.warning(
            "compaction.compaction_pressure_pct=%g clamped to %g (valid range %g-%g)",
            pct,
            clamped,
            COMPACTION_PRESSURE_MIN_PCT,
            COMPACTION_PRESSURE_MAX_PCT,
        )
    return clamped


COMPACTION_PRESSURE_PCT = _load_compaction_pressure_pct()
COMPACTION_PRESSURE_RATIO = COMPACTION_PRESSURE_PCT / 100.0
...
def _append_trim_event(
    ...
    *,
    ...
    units_before: int,
    units_after: int,
    chars_trimmed: Optional[int],
) -> None:
    """Best-effort trim telemetry append; never raises."""
    try:
        ...
        sink.append(
            {
                ...
                "chars_trimmed": (int(chars_trimmed) if chars_trimmed is not None else None),
            }
        )
    except Exception:
        return
...
def purge_ctc_compaction_jsonl(storage_dir: str = "", vodka_subdir: str = "vodka") -> int:
    """
    Delete all CTC compaction and breadcrumb JSONL files.
    Intended for router startup / >>flush so each boundary starts clean.
    Returns number of deleted files.
    """
    deleted = 0
    scoped_root, legacy_root = _resolve_vodka_roots(storage_dir, vodka_subdir)
    folders = [
        os.path.join(scoped_root, "ctc_compaction"),
        os.path.join(scoped_root, "ctc_breadcrumbs"),
    ]
    legacy_folders = [
        os.path.join(legacy_root, "ctc_compaction"),
        os.path.join(legacy_root, "ctc_breadcrumbs"),
    ]
    for legacy_folder in legacy_folders:
        if all(os.path.normcase(legacy_folder) != os.path.normcase(folder) for folder in folders):
            folders.append(legacy_folder)
...
class FastRecall:
    ...
    def janitor_if_due(self, interval_seconds: int = 3600, *, trim_event_sink: Any = None, current_turn: int = 0):
        now = time.time()
        if now - self._last_janitor_run < interval_seconds:
            return
        self._last_janitor_run = now
        self._janitor(trim_event_sink=trim_event_sink, current_turn=current_turn)

    def _janitor(self, *, trim_event_sink: Any = None, current_turn: int = 0):
        data = self.S.load_facts()
        now = self._now()
        units_before_all = sum(1 for rec in data.values() if isinstance(rec, dict) and rec.get("type") == "vodka_ctx")
        ...
        for k, rec in data.items():
            ...
            if exp_dt and exp_dt < now:
                to_delete.append(k)
        for k in to_delete:
            before = units_before_all
            self.S.log(f"VODKA_JANITOR_EXPIRE - {k}")
            self.S.debug_log(f"JANITOR: {k}")
            data.pop(k, None)
            units_before_all = max(0, units_before_all - 1)
            _append_trim_event(
                trim_event_sink,
                turn=current_turn,
                reason="JANITOR_EXPIRE",
                item_id=k,
                units_before=before,
                units_after=units_before_all,
                chars_trimmed=None,
            )
    ...
    def nuke(self, *, trim_event_sink: Any = None, current_turn: int = 0):
        data = self.S.load_facts()
...
class Filter:
    ...
    def __init__(self):
        ...
        print(f"[vodka_init] instance created enable_summary={self.valves.enable_summary}", flush=True)
        ...
        self._ctc_last_returned_ctx_lock = threading.Lock()
        self._ctc_last_returned_ctx_by_session: Dict[str, str] = {}
    ...
    def _handle_control_commands(
        ...
        if norm_no_ws in ("!!nuke", "nuke!!"):
            fr.nuke(
                trim_event_sink=body.get("_trim_event_sink"),
                current_turn=int(body.get("_current_turn", 0) or 0),
            )
    ...
    def _ctc_compaction_file(self, session_id: str) -> str:
        fr = self._get_storage_and_fr()
        sid = self._normalize_session_id(session_id)
        folder = _ensure_dir(os.path.join(fr.S.base, "ctc_compaction"))
        return os.path.join(folder, f"{sid}.jsonl")

    def _ctc_compaction_file_legacy(self, session_id: str) -> str:
        fr = self._get_storage_and_fr()
        sid = self._normalize_session_id(session_id)
        folder = _ensure_dir(os.path.join(fr.S.legacy_base, "ctc_compaction"))
        return os.path.join(folder, f"{sid}.jsonl")

    def _ctc_compaction_read_file(self, session_id: str) -> str:
        primary = self._ctc_compaction_file(session_id)
        if os.path.exists(primary):
            _LOG.warning(
                "[ctc_read_result] session_id=%s path=%s exists=%s record_count=%s",
                session_id,
                primary,
                True,
                len(self._load_ctc_compaction_jsonl(primary)),
            )
            return primary
        legacy = self._ctc_compaction_file_legacy(session_id)
        if os.path.exists(legacy):
            _LOG.warning(
                "[ctc_read_result] session_id=%s path=%s exists=%s record_count=%s",
                session_id,
                legacy,
                True,
                len(self._load_ctc_compaction_jsonl(legacy)),
            )
            return legacy
        _LOG.warning(
            "[ctc_read_result] session_id=%s path=%s exists=%s record_count=%s",
    ...
    def _ctc_breadcrumb_file(self, session_id: str) -> str:
        fr = self._get_storage_and_fr()
        sid = self._normalize_session_id(session_id)
        folder = _ensure_dir(os.path.join(fr.S.base, "ctc_breadcrumbs"))
        return os.path.join(folder, f"{sid}.jsonl")

    def _ctc_breadcrumb_file_legacy(self, session_id: str) -> str:
        fr = self._get_storage_and_fr()
        sid = self._normalize_session_id(session_id)
        folder = _ensure_dir(os.path.join(fr.S.legacy_base, "ctc_breadcrumbs"))
        return os.path.join(folder, f"{sid}.jsonl")

    def _ctc_breadcrumb_read_file(self, session_id: str) -> str:
        primary = self._ctc_breadcrumb_file(session_id)
        if os.path.exists(primary):
            return primary
        legacy = self._ctc_breadcrumb_file_legacy(session_id)
        if os.path.exists(legacy):
            return legacy
        return primary

    def _load_ctc_compaction_jsonl(self, path: str) -> List[Dict[str, Any]]:
            ...
            with open(path, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    try:
                        ...
                        continue
                    if isinstance(rec, dict) and rec.get("snapshot_id"):
                        out.append(rec)
        except Exception:
            return []
        return out

    def _load_ctc_breadcrumb_jsonl(self, path: str) -> List[Dict[str, Any]]:
        out: List[Dict[str, Any]] = []
        try:
            with open(path, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    try:
                        ...
                        continue
                    if isinstance(rec, dict) and rec.get("ctx_id") and rec.get("text"):
                        out.append(rec)
        except Exception:
            return []
        return out

    def _write_ctc_compaction_jsonl(self, path: str, records: List[Dict[str, Any]]) -> None:
        tmp = path + ".tmp"
        with open(tmp, "w", encoding="utf-8") as f:
            for rec in records:
                f.write(_dumps(rec) + "\n")
    ...
    def _write_ctc_breadcrumb_jsonl(self, path: str, records: List[Dict[str, Any]]) -> None:
        tmp = path + ".tmp"
        with open(tmp, "w", encoding="utf-8") as f:
            for rec in records:
                f.write(_dumps(rec) + "\n")
    ...
    def _append_ctc_compaction_record(self, session_id: str, record: Dict[str, Any]) -> None:
        path = self._ctc_compaction_file(session_id)
        _LOG.warning(
            "[ctc_write_path] session_id=%s path=%s",
            session_id,
            path,
        )
        records = self._load_ctc_compaction_jsonl(path)
        records.append(record)
        max_sessions = 64
        max_bytes = 256 * 1024
        # Oldest-first pruning by record count and byte budget.
        while len(records) > max_sessions:
            records.pop(0)
        while records and len(_dumps(records).encode("utf-8")) > max_bytes:
            records.pop(0)
        self._write_ctc_compaction_jsonl(path, records)

    def _append_ctc_breadcrumb_record(self, session_id: str, record: Dict[str, Any]) -> None:
        path = self._ctc_breadcrumb_file(session_id)
        records = self._load_ctc_breadcrumb_jsonl(path)
        records.append(record)
        max_entries = 64
        max_bytes = 256 * 1024
        ...
        while records and len(_dumps(records).encode("utf-8")) > max_bytes:
            records.pop(0)
        self._write_ctc_breadcrumb_jsonl(path, records)

    def _write_ctc_breadcrumb_sentences(
        self,
        session_id: str,
        *,
        sentences: List[str],
        raw_text: str,
        turn_range: Dict[str, Any],
        snapshot_id: str = "",
    ) -> int:
        path = self._ctc_breadcrumb_file(session_id)
        raw_len = len(str(raw_text or ""))
        written = 0
        for sentence in [s.strip() for s in (sentences or []) if str(s or "").strip()]:
            ctx_id = _sha256_crc_id(sentence)
            record: Dict[str, Any] = {
                "ctx_id": ctx_id,
                "kind": "compaction_anchor",
                "text": sentence,
                "raw_text": str(raw_text or ""),
                "turn_range": dict(turn_range or {}),
                "session_id": session_id,
                "created_at": _iso_ts(),
            }
            if snapshot_id:
                record["compaction_snapshot_id"] = snapshot_id
            log_snapshot_id = snapshot_id or f"ingest_turn_{int((turn_range or {}).get('last') or 0)}"
            try:
                _LOG.warning(
                    "[ctc_breadcrumb_text] session_id=%s snapshot_id=%s ctx_id=%s text=%s",
                    session_id,
                    log_snapshot_id,
                    ctx_id,
                    sentence,
                )
                self._append_ctc_breadcrumb_record(session_id, record)
                breadcrumb_count = len(self._load_ctc_breadcrumb_jsonl(path))
                _LOG.warning(
                    "[ctc_breadcrumb_write] session_id=%s snapshot_id=%s ctx_id=%s extracted_chars=%d raw_chars=%d write_ok=1 record_count=%d path=%s",
                    session_id,
                    log_snapshot_id,
                    ctx_id,
                    int(len(sentence)),
                ...
                _LOG.warning(
                    "[ctc_breadcrumb_write] session_id=%s snapshot_id=%s ctx_id=%s extracted_chars=%d raw_chars=%d write_ok=0 path=%s exc=%r",
                    session_id,
                    log_snapshot_id,
                    ctx_id,
                    int(len(sentence)),
    ...
    def _compact_session_text(self, messages: List[Dict[str, Any]], *, max_turn_pairs: int = 4, per_turn_max_chars: int = 240) -> List[str]:
        lines: List[str] = []
        turns = [
        ...
        # Keep only the most recent turns for live state.
        for m in turns[-max(1, int(max_turn_pairs)) * 2:]:
            role = "User" if str(m.get("role") or "").lower() == "user" else "Assistant"
            lines.append(f"{role}: {self._compact_memory_text(m.get('content', ''), max_len=per_turn_max_chars)}")
        return [ln for ln in lines if ln]

    def _extract_first_sentence(self, text: str, *, max_len: int = 220) -> str:
        t = " ".join(str(text or "").strip().split())
        if not t:
            return ""
        t = self._strip_anchor_preamble(t)
        sentences = [s.strip() for s in re.split(r"(?<=[.!?])\s+", t) if s.strip()]
        out = ""
        for sent in sentences:
            low = sent.lower()
            if any(k in low for k in ("excerpt", "continuing", "public domain", "project gutenberg", "moby dick excerpt")):
                continue
            if re.match(r"^chapter\s+\d+\b", low):
                continue
            if re.match(r"^chapter\s+[ivxlcdm]+\b", low):
                continue
            out = sent
            break
        if not out:
            out = t[:max_len].strip()
        return self._compact_memory_text(out, max_len=max_len)

    def _strip_anchor_preamble(self, text: str) -> str:
        t = " ".join(str(text or "").strip().split())
        if not t:
            return ""
        t = re.sub(r"^\[[^\]]+\]\s*", "", t)
        ...
        while len(sentences) > 1 and any(k in sentences[0].lower() for k in skip_words):
            sentences.pop(0)
        return " ".join(sentences)

    def _extract_anchor_facts(self, messages: List[Dict[str, Any]], *, max_items: int = 8, max_total_chars: int = 512) -> List[str]:
        ua = [
            m for m in (messages or [])
        ...
        def _add_fact(value: str) -> None:
            ...
            fact = self._compact_memory_text(value, max_len=160).strip()
            key = " ".join(fact.lower().split())
            if not fact or key in seen:
                return
            if len(fact) <= 8 and fact.upper() == fact and any(ch.isalpha() for ch in fact):
                return
            if fact.lower() in {"must", "yes", "no", "ok", "okay", "true", "false", "n/a", "na"}:
                return
            if total_chars + len(fact) > max_total_chars:
                return
            seen.add(key)
            facts.append(fact)
            total_chars += len(fact)

        # Opening anchor: first sentence of the first user turn.
        _add_fact(f"Opening line: {self._extract_first_sentence(str(ua[0].get('content') or ''), max_len=180)}")

        quoted_pat = re.compile(r"[\"“”'‘’]([^\"“”'‘’]{2,80})[\"“”'‘’]")
        ent_pat = re.compile(r"\b(?:[A-Z][a-z]{2,}|[A-Z]{2,})(?:\s+(?:[A-Z][a-z]{2,}|[A-Z]{2,}))*\b")
        stop_single = {
            "The", "This", "That", "These", "Those", "And", "But", "Or", "For", "With", "From",
            "When", "Where", "What", "Who", "Why", "How", "Call", "Chapter", "Continuing",
            "Moby", "Dick", "User", "Assistant", "Turn",
        }
        stop_multi = {
            "chapter", "continuing", "user", "assistant", "turn", "session", "smoke", "excerpt",
            "moby dick excerpt", "public domain", "the following", "the text",
        }
        ...
        for msg in ua[:12]:
            ...
            for q in quoted_pat.findall(txt):
                qn = " ".join(q.split()).strip()
                if len(qn) >= 2:
                    key = " ".join(qn.lower().split())
                    entity_counts[key] = int(entity_counts.get(key, 0) + 1)
                    entity_first_seen.setdefault(key, seq_index)
                    entity_candidates.append(qn)
                    seq_index += 1
            for ent in ent_pat.findall(txt):
                ent_norm = " ".join(ent.split()).strip(" ,.;:()[]{}\"'")
                if not ent_norm:
                    continue
                ent_low = ent_norm.lower()
                if ent_low in stop_multi:
                    continue
                if ent_norm in stop_single:
                    continue
                # Require at least one non-generic capitalized token.
                parts = ent_norm.split()
                if len(parts) == 1 and ent_norm in stop_single:
                    continue
                if len(parts) == 1 and len(ent_norm) < 4:
                    continue
                key = " ".join(ent_norm.lower().split())
                entity_counts[key] = int(entity_counts.get(key, 0) + 1)
                entity_first_seen.setdefault(key, seq_index)
                entity_candidates.append(ent_norm)
                seq_index += 1

        # Prefer high-signal anchors from explicit mentions and named entities.
        ranked: List[Tuple[int, int, int, str]] = []
        for cand in entity_candidates:
            key = " ".join(cand.lower().split())
            cnt = int(entity_counts.get(key, 1))
            first = int(entity_first_seen.get(key, 0))
            ranked.append((-cnt, first, -len(cand), cand))
        for _, __, ___, cand in sorted(ranked):
            if len(facts) >= max_items:
                break
            low = cand.lower()
            if low in {"call me ishmael", "call me ishmael."}:
                _add_fact("Opening line: Call me Ishmael.")
                continue
            if low in {"pequod", "ahab", "queequeg", "moby dick", "white whale"}:
                _add_fact(cand)
                continue
            _add_fact(cand)

        # Lightweight explicit role anchors from the session text.
        joined = "\n".join(str(m.get("content") or "") for m in ua[:12])
        role_patterns = [
            (r"\bship(?:'s)?\s+name\s+is\s+([A-Z][A-Za-z0-9\-\s]{2,80})", "Ship"),
            (r"\bcaptain\s+is\s+([A-Z][A-Za-z0-9\-\s]{2,80})", "Captain"),
            (r"\bharpooner(?:\s+that\s+ismael\s+shared\s+a\s+bed\s+with)?\s+is\s+([A-Z][A-Za-z0-9\-\s]{2,80})", "Harpooner"),
            (r"\bquarry\s+is\s+([A-Z][A-Za-z0-9\-\s]{2,80})", "Quarry"),
            (r"\b([A-Z][A-Za-z0-9\-]{2,40})'s\s+(?:arm|bed|hand|voice|face|name)\b", "Named anchor"),
        ]
        for pat, label in role_patterns:
            ...
            if m:
                val = self._compact_memory_text(m.group(1), max_len=120).strip(" ,.;:")
                if val:
                    if label == "Named anchor" and val.lower() not in {"the", "this", "that"}:
                        _add_fact(val)
                    else:
                        _add_fact(f"{label}: {val}")

        # Deduplicate one more time while preserving order.
        deduped: List[str] = []
        seen2: set[str] = set()
        for fact in facts:
            key = " ".join(fact.lower().split())
    ...
    def _extract_session_facts(self, messages: List[Dict[str, Any]], max_chars: int = 150) -> str:
        turns = [
            m for m in (messages or [])
            if str(m.get("role") or "").lower() in ("user", "assistant")
            and isinstance(m.get("content", ""), str)
        ...
        skip_prefix_re = re.compile(r"^\s*\[(?:SESSION|SMOKE)-CHUNK-[^\]]+\]\s*", re.IGNORECASE)
        cfg_key_re = re.compile(r"\b[a-z][a-z0-9_]{2,}\b\s*(?:[:=]|\b(?:is|are|lives in|lives under|lives at)\b)\s*[^,;]{1,80}", re.IGNORECASE)
        unit_re = re.compile(r"\b\d+(?:\.\d+)?\s*(?:kb|mb|kib|gb|%|chars?|bytes?)\b", re.IGNORECASE)
        command_re = re.compile(r"(?:^|[\s`])(>>[a-z0-9][a-z0-9_-]*)", re.IGNORECASE)
        schema_item_re = re.compile(r"\b[a-z][a-z0-9_]{2,}\b")
        metadata_line_re = re.compile(r"\b(?:Confidence|Source|Profile|Sarc|Snark)\s*:", re.IGNORECASE)
        status_block_re = re.compile(
            r"(?:ctc_compaction_debug|current pressure:|compaction threshold:|headroom:|session_id=|"
            r"kaioken_enabled=|vodka_preset=|last_hits=|record_count=|snapshot_id=)",
            re.IGNORECASE,
        )
        filler = {
            "ok", "okay", "yes", "yep", "sure", "right", "thanks", "thank you", "got it", "roger",
            "no", "nope", "fine", "cool", "good", "great", "sure thing",
        }
        wanted_phrases = (
            "decided:",
            "constraint:",
        ...
        def _score_sentence(sent: str, role: str) -> Tuple[int, str]:
            text = " ".join(str(sent or "").split()).strip()
            if not text:
                return 0, ""
            text = skip_prefix_re.sub("", text).strip()
            if not text:
                return 0, ""
            low = text.lower()
            words = [w for w in re.split(r"\s+", low) if w]
            if len(words) <= 1 or low in filler:
                return 0, ""
            if low.startswith("[session-chunk-") or low.startswith("[smoke-chunk-"):
                return 0, ""
            if re.fullmatch(r"[\W_]+", text):
                return 0, ""
            if metadata_line_re.search(text):
                return 0, ""
            ...
            score = 0
            focus_bonus = 0
            def _focused_snippet(raw_text: str, low_text: str) -> str:
                if "compaction_pressure_pct" in low_text or "router-config.yaml" in low_text or "compaction:" in low_text:
                    return "compaction_pressure_pct under compaction: router-config.yaml"
                if "ctx_id" in low_text and ("canonical" in low_text or "pointer" in low_text):
                    return "ctx_id canonical; pointer not stored"
                if ">>summ" in low_text:
                    return ">>summ filesystem KB ingest pipeline; not a compaction helper"
                if ("64" in low_text and "256" in low_text and "8" in low_text and ("kib" in low_text or "kb" in low_text)):
                    return "64 / 256 KiB / 8 KiB retention ceilings"
                if (
                    "incorrectly mapped" in low_text
                    or "incorrectly conflated" in low_text
                    or "manual memory lane" in low_text
                    or "pre-compaction flush" in low_text
                    or "rolling window trim path" in low_text
                    or (("!!" in raw_text or "??" in raw_text) or "double-bang" in low_text or "double-question" in low_text) and "ctc" in low_text
                ):
                    return "Vodka !!/?? manual memory lane vs CTC rolling window"
                return ""
            focus_text = _focused_snippet(text, low)
            # Heavily down-rank schema enumerations / field lists.
            parts = [p.strip() for p in re.split(r",\s*", text) if p.strip()]
            schema_like_parts = 0
            for part in parts:
                if schema_item_re.search(part) and (
                    ":" in part
                    or "=" in part
                    ...
                    schema_like_parts += 1
            schema_enumeration = len(parts) >= 4 and schema_like_parts >= 4
            if schema_enumeration:
                has_focus_signal = (
                    "compaction_pressure_pct" in low
                    or "router-config.yaml" in low
                    or "compaction:" in low
                    or "ctx_id" in low
                    or "canonical" in low
                    or ">>summ" in low
                    or (unit_re.search(text) is not None and (
                        re.search(r"\b64\b", text) is not None
                        or re.search(r"\b256\s*(?:kib|kb)\b", low) is not None
                        or re.search(r"\b8\s*(?:kib|kb)\b", low) is not None
                    ))
                )
            ...
            if "compaction_pressure_pct" in low:
                focus_bonus += 90
            if "router-config.yaml" in low:
                focus_bonus += 40
            if "compaction:" in low:
                focus_bonus += 35
            if "ctx_id" in low:
                focus_bonus += 85
            if "canonical" in low:
                focus_bonus += 35
            if "pointer" in low and "not stored" in low:
                focus_bonus += 35
            if ">>summ" in low:
                focus_bonus += 90
            if "filesystem" in low and "ingest" in low:
                focus_bonus += 35
            if "not a compaction helper" in low:
                focus_bonus += 65
            if "incorrectly mapped" in low:
                focus_bonus += 95
            if "incorrectly conflated" in low:
                focus_bonus += 105
            if "pre-compaction flush" in low:
                focus_bonus += 70
            if "rolling window trim path" in low:
                focus_bonus += 65
            if "manual memory lane" in low:
                focus_bonus += 85
            if "!!" in text or "??" in text:
                focus_bonus += 30
            if unit_re.search(text):
                retention_hits = 0
                if re.search(r"\b64\b", text):
                    retention_hits += 1
                if re.search(r"\b256\s*(?:kib|kb)\b", low):
                    retention_hits += 1
                if re.search(r"\b8\s*(?:kib|kb)\b", low):
                    retention_hits += 1
                if retention_hits >= 2:
                    focus_bonus += 70 + (retention_hits * 10)
            ...
            if "confirmed" in low:
                score += 22
            if "must not" in low or "do not" in low:
                score += 24
            if "instead" in low:
                score += 16
            if "not a" in low:
            ...
            if command_re.search(text):
                score += 26
            if cfg_key_re.search(text):
                score += 20
            if unit_re.search(text):
                score += 18
            if re.search(r"\b(?:[A-Za-z_][A-Za-z0-9_]*\s*[:=]\s*[^,;]{1,40})", text):
                score += 12
            if any(ch.isdigit() for ch in text):
                score += 6
            if role == "assistant" and any(k in low for k in ("decision", "choose", "plan", "route", "use ", "we will", "must")):
                score += 8
            if role == "user" and any(k in low for k in ("correction", "confirmed", "constraint", "instead", "must not", "do not")):
                score += 8
            score += focus_bonus
            if focus_text:
                text = focus_text
            score += min(20, len(set(self._tokenize_for_search(text))))
            score += min(12, len(text) // 40)
            return score, text
        ...
        for msg in turns:
            ...
            content = self._strip_anchor_preamble(str(msg.get("content") or "").strip())
            if not content:
                continue
            ...
            if not parts:
                parts = [content]
            for sent in parts:
                score, clean = _score_sentence(sent, role)
                if not clean or score <= 0:
                    continue
                candidates.append((score, seq, clean))
                seq += 1

        if not candidates:
            # Minimum floor: if the high-signal scorer filtered everything out,
            # still return a couple of concrete sentences instead of empty text.
            ...
            for msg in turns:
                ...
                for sent in parts:
                    ...
                    if low.startswith("[session-chunk-") or low.startswith("[smoke-chunk-"):
                        continue
                    if low in filler or re.fullmatch(r"[\W_]+", text):
                        continue
                    if status_block_re.search(text):
                        continue
                    fallback.append((1, seq, text))
                    seq += 1
            if not fallback:
                first = self._extract_first_sentence(str(turns[0].get("content") or ""), max_len=180)
                return first[:max_chars].strip()
            fallback.sort(key=lambda item: (-item[0], item[1], len(item[2])))
            picked: List[str] = []
            used = 0
            seen: set[str] = set()
            for _, __, sent in fallback:
                key = " ".join(sent.lower().split())
                if not sent or key in seen:
                    continue
                if used + len(sent) > max_chars and picked:
                    continue
                if len(sent) > max_chars and not picked:
                ...
                picked.append(sent)
                used += len(sent)
                if len(picked) >= min_items or used >= max_chars:
                    break
            return "\n".join(picked).strip()
        ...
        if len(picked) < min_items:
            ...
            for msg in turns:
                ...
                for sent in parts:
                    ...
                    if key in backfill_seen:
                        continue
                    if used + len(text) > max_chars and picked:
                        continue
                    if len(text) > max_chars and not picked:
                        text = text[:max_chars].rstrip()
                    if not text:
                        continue
                    backfill_seen.add(key)
                    backfill.append(text)
                    used += len(text)
                    if len(picked) + len(backfill) >= min_items or used >= max_chars:
                        break
                if len(picked) + len(backfill) >= min_items or used >= max_chars:
                    break
            if backfill:
    ...
    def _find_last_turn_by_role(self, messages: List[Dict[str, Any]], role: str) -> str:
        for m in reversed(messages or []):
            if str(m.get("role") or "").lower() != role:
                continue
            c = m.get("content", "")
    ...
    def _make_compaction_snapshot(
        ...
        *,
        ...
        _LOG.warning(
            "[ctc_compaction_enter] session_id=%s pressure_pct=%s trigger=%s",
            session_id,
            f"{float(pressure_pct):.2f}",
            trigger,
        )

        def _strip_footer_marker_suffix(value: Any) -> str:
            raw = str(value or "")
            if not raw.strip():
                return ""
            marker_re = re.compile(r"(?is)(?:\|\s*)?(?:confidence:|source:|profile:).*\Z")
        ...
        if not ua:
            return None
        now = _iso_ts()
        task_lines = self._compact_session_text(ua, max_turn_pairs=4, per_turn_max_chars=180)
        task_state = " | ".join(task_lines)
        last_corr = ""
        last_decision = ""
        anchor_facts = self._extract_anchor_facts(ua, max_items=6, max_total_chars=512)
        snapshot_id = _sha256_crc_id(f"{session_id}|{current_turn}|{now}|{task_state}")
        omitted_spans: List[Dict[str, Any]] = []
        if len(ua) > 4:
            omitted = ua[:-4]
            omitted_for_breadcrumbs = [
                m for m in omitted
                if str(m.get("role") or "").lower() == "assistant"
                and not (
                    str(m.get("content") or "").lstrip().startswith("[status]")
                    or "kaioken_enabled=" in str(m.get("content") or "")
                )
            ]
            if omitted_for_breadcrumbs:
                raw_omitted_text = "\n".join(
                    f"Assistant: {self._compact_memory_text(m.get('content', ''), max_len=180)}"
                    for m in omitted_for_breadcrumbs
                )
                breadcrumb_path = self._ctc_breadcrumb_file(session_id)
                omitted_turn_range = {"first": 1, "last": max(1, len(omitted))}
                ingestion_cutoff_turn = max(1, int(current_turn or 0) - 2)
                existing_breadcrumbs = self._load_ctc_breadcrumb_jsonl(breadcrumb_path)
                ingestion_breadcrumbs = [
                    rec
                    for rec in existing_breadcrumbs
                    if str(rec.get("session_id") or "").strip() == session_id
                    and not str(rec.get("compaction_snapshot_id") or "").strip()
                    and int((rec.get("turn_range") or {}).get("last") or 0) < ingestion_cutoff_turn
                ]
                if ingestion_breadcrumbs:
                    for rec in ingestion_breadcrumbs:
                        cid = str(rec.get("ctx_id") or "").strip()
                        if not cid:
                            continue
                        omitted_spans.append(
                            {
                                "ctx_id": cid,
                                "turn_range": dict(rec.get("turn_range") or omitted_turn_range),
                                "kind": str(rec.get("kind") or "compaction_anchor"),
                            }
                        )
                else:
                    breadcrumb_source_text = "\n".join(
                        f"{'User' if str(m.get('role') or '').lower() == 'user' else 'Assistant'}: {self._compact_memory_text(m.get('content', ''), max_len=180)}"
                        for m in omitted_for_breadcrumbs
                    )
                    breadcrumb_text = self._extract_session_facts(omitted_for_breadcrumbs, max_chars=150).strip()
                    if not breadcrumb_text:
                        breadcrumb_text = self._compact_memory_text(breadcrumb_source_text, max_len=150).strip()
                    breadcrumb_sentences = [s.strip() for s in breadcrumb_text.split("\n") if s.strip()]
                    if breadcrumb_sentences:
                        written = self._write_ctc_breadcrumb_sentences(
                            session_id,
                            sentences=breadcrumb_sentences,
                            raw_text=raw_omitted_text,
                            turn_range=omitted_turn_range,
                            snapshot_id=snapshot_id,
                        )
                        if written > 0:
                            refreshed = self._load_ctc_breadcrumb_jsonl(breadcrumb_path)
                            for rec in refreshed[-written:]:
                                cid = str(rec.get("ctx_id") or "").strip()
                                if not cid:
                                    continue
                                omitted_spans.append(
        ...
        for m in reversed(ua):
            role = str(m.get("role") or "").lower()
            txt = str(m.get("content") or "").strip()
            low = txt.lower()
            if not txt:
                continue
            if not last_corr and role == "user" and any(k in low for k in ("correction", "actually", "no, ", "not that", "that's wrong", "wrong", "instead")):
                last_corr = self._compact_memory_text(txt, max_len=512)
            if not last_decision and role == "assistant" and any(k in low for k in ("decision", "choose", "we will", "plan", "use ", "route", "do this")):
                last_decision = self._compact_memory_text(txt, max_len=512)
            if last_corr and last_decision:
                break

        if not task_state:
            task_state = self._compact_memory_text(self._find_last_turn_by_role(ua, "assistant") or self._find_last_turn_by_role(ua, "user"), max_len=1024)
        task_state = self._compact_memory_text(task_state, max_len=1024)
        task_state = _strip_footer_marker_suffix(task_state)
        if len(task_state) > 1024:
            task_state = task_state[:1024].rstrip()
        last_corr = self._compact_memory_text(last_corr, max_len=512) if last_corr else ""
        last_corr = _strip_footer_marker_suffix(last_corr)
        last_decision = self._compact_memory_text(last_decision, max_len=512) if last_decision else ""
        last_decision = _strip_footer_marker_suffix(last_decision)
        if not last_corr:
            last_corr = None
        ...
        snap = {
            ...
            "session_id": session_id,
            "created_at": now,
            "turn_range": {"first": max(1, current_turn - max(0, len(ua) - 1)), "last": max(1, current_turn)},
            "ctx_size_at_compaction": int(ctx_size or 0),
            "trigger": str(trigger or "pressure"),
            "task_state": task_state,
            "last_correction": last_corr,
            "last_decision": last_decision,
            "anchor_facts": anchor_facts,
            "omitted_spans": omitted_spans,
            "provenance": {
                "model_id": "router",
                "ctx_pressure_pct": 0.0,
                "pressure_threshold_pct": float(COMPACTION_PRESSURE_PCT),
            },
        }
        try:
            total_chars = sum(len(str(m.get("content", "") or "")) for m in ua)
            pressure_pct = 0.0
            if ctx_size and int(ctx_size) > 0:
                pressure_pct = round(min(1.0, total_chars / max(1, int(ctx_size))) * 100.0, 2)
            snap["provenance"]["ctx_pressure_pct"] = pressure_pct
        except Exception:
            pass
        return snap

    def _make_compaction_message(self, snap: Dict[str, Any]) -> str:
        spans = snap.get("omitted_spans") or []
        ptrs = []
        for s in spans:
            cid = str(s.get("ctx_id") or "").strip()
            if cid:
                ptrs.append(f"[ctx:{cid}]")
        lines = [
            f"{CTC_COMPACTION_PREFIX}AUTHORITATIVE SESSION SNAPSHOT {str(snap.get('snapshot_id') or '').strip()} turn {snap.get('turn_range', {}).get('first', 0)}-{snap.get('turn_range', {}).get('last', 0)}",
            "This snapshot is the authoritative session state for content that occurred before this boundary.",
            "For questions about earlier conversation content, consult this snapshot first.",
            "Do not prefer web/wiki retrieval unless this snapshot does not contain the needed fact.",
            "All session-specific facts in this session must come from this snapshot only. Do not generate session-specific facts from training knowledge or priors.",
            "If a session-specific fact is not in this snapshot, say so rather than inventing it or falling back to web/wiki for that fact.",
            f"task_state: {str(snap.get('task_state') or '').strip()}",
        ]
        if snap.get("last_correction"):
            lines.append(f"last_correction: {str(snap.get('last_correction') or '').strip()}")
        if snap.get("last_decision"):
            lines.append(f"last_decision: {str(snap.get('last_decision') or '').strip()}")
        anchors = snap.get("anchor_facts") or []
        if anchors:
            lines.append("anchor_facts from this session:")
            for a in anchors:
                lines.append(f"- {str(a).strip()}")
        if ptrs:
            lines.append("breadcrumbs: " + " ".join(ptrs))
        return "\n".join(lines).strip()

    def _maybe_compact_ctc_history(
        self,
        messages: List[Dict[str, Any]],
        *,
        session_id: str,
        ...
        total_chars = sum(len(str(m.get("content", "") or "")) for m in ua)
        # Heuristic boundary pressure: compact early enough to avoid cliff-edge trim.
        pressure_pct = (total_chars / max(1, int(ctx_size or max_chars or 1))) if max(ctx_size or max_chars or 0, 0) > 0 else 0.0
        _LOG.warning(
            "[ctc_pressure_inputs] session_id=%s total_chars=%s max_chars=%s ctx_size=%s",
            session_id,
            total_chars,
            max_chars,
            ctx_size,
        ...
        should_compact = bool(
            pressure_pct >= COMPACTION_PRESSURE_RATIO
            or current_turn > 0 and current_turn % 12 == 0
            or total_chars > max(0, int(max_chars or 0) * 0.80)
        )
        _LOG.warning(
            "[ctc_pressure_check] pressure_pct=%s threshold=%s will_compact=%s",
            f"{float(pressure_pct) * 100.0:.2f}",
            f"{float(COMPACTION_PRESSURE_PCT):g}",
            "True" if should_compact else "False",
        )
        if not should_compact:
            return messages, False

        snap = self._make_compaction_snapshot(
            session_id=session_id,
            messages=messages,
            ctx_size=ctx_size,
            trigger=(
                f"pressure_{COMPACTION_PRESSURE_PCT:g}"
                if pressure_pct >= COMPACTION_PRESSURE_RATIO
                else ("turn_backstop" if current_turn % 12 == 0 else "session_end")
            ),
            current_turn=current_turn,
            pressure_pct=pressure_pct * 100.0,
        )
        _LOG.warning(
            "[ctc_snap_result] session_id=%s snap_is_none=%s",
            session_id,
            snap is None,
        )
        if not snap:
            return messages, False
        try:
            self._append_ctc_compaction_record(session_id, snap)
        except Exception:
            pass

        compaction_msg = {"role": "system", "content": self._make_compaction_message(snap)}
        # Preserve system messages; prepend the new compact snapshot, keep
        # only the last few conversational turns to reduce prompt pressure.
        system = [m for m in messages if m.get("role") == "system"]
        recent = [m for m in messages if m.get("role") in ("user", "assistant")][-8:]
        return system + [compaction_msg] + recent, True
    ...
    def get_ctc_compaction_status(self, session_id: str, *, user_turn_hint: int = 0) -> Dict[str, Any]:
        """Return lightweight observability for CTC compaction behavior."""
        sid = self._normalize_session_id(session_id)
        path = self._ctc_compaction_read_file(sid)
        records = self._load_ctc_compaction_jsonl(path)
        breadcrumb_path = self._ctc_breadcrumb_read_file(sid)
        breadcrumb_records = self._load_ctc_breadcrumb_jsonl(breadcrumb_path)
        last = records[-1] if records else {}
        last_breadcrumb = breadcrumb_records[-1] if breadcrumb_records else {}
        turn_mod = 12
        current_turn = int(max(0, int(user_turn_hint or 0)))
        turns_to_backstop = None
        if current_turn > 0:
            rem = current_turn % turn_mod
            turns_to_backstop = turn_mod if rem == 0 else (turn_mod - rem)
        return {
            "session_id": sid,
            "enabled": True,
            "raw_threshold_pct": COMPACTION_PRESSURE_RAW,
            "threshold_pct": float(COMPACTION_PRESSURE_PCT),
            "threshold_ratio": float(COMPACTION_PRESSURE_RATIO),
            "turn_backstop_every": turn_mod,
            "turn_backstop_remaining": turns_to_backstop,
            "record_count": int(len(records)),
            "breadcrumb_record_count": int(len(breadcrumb_records)),
            "last_snapshot_id": str(last.get("snapshot_id") or ""),
            "last_trigger": str(last.get("trigger") or ""),
            "last_ctx_pressure_pct": float((last.get("provenance") or {}).get("ctx_pressure_pct") or 0.0),
            "last_ctx_size_at_compaction": int(last.get("ctx_size_at_compaction") or 0),
            "last_turn_first": int((last.get("turn_range") or {}).get("first") or 0),
            "last_turn_last": int((last.get("turn_range") or {}).get("last") or 0),
            "last_breadcrumb_id": str(last_breadcrumb.get("ctx_id") or ""),
            "compaction_file": path,
            "breadcrumb_file": breadcrumb_path,
            "compaction_file_exists": bool(os.path.exists(path)),
            "breadcrumb_file_exists": bool(os.path.exists(breadcrumb_path)),
            "user_turn_hint": current_turn,
        }

    def get_latest_ctc_compaction_snapshot(self, session_id: str) -> Dict[str, Any]:
        """Return the most recent CTC compaction snapshot for a session."""
        sid = self._normalize_session_id(session_id)
        path = self._ctc_compaction_read_file(sid)
        _LOG.warning(
            "[ctc_lookup_path] session_id=%s path=%s exists=%s",
            session_id,
            path,
            os.path.exists(path),
        )
    ...
    def _compact_compaction_query_text(self, value: Any, max_len: int = 220) -> str:
        txt = str(value or "").strip().replace("\n", " ")
        if len(txt) > max_len:
            txt = txt[: max(0, max_len - 3)].rstrip() + "..."
        return txt

    def _split_compaction_sentences(self, text: str) -> List[str]:
        raw = " ".join(str(text or "").split())
        if not raw:
            return []
        return [s.strip() for s in re.split(r"(?<=[.!?])\s+", raw) if s.strip()]

    def _compaction_query_tokens(self, text: str) -> set[str]:
        toks = set()
        for tok in re.findall(r"[A-Za-z0-9][A-Za-z0-9_\-']*", str(text or "").lower()):
            if len(tok) < 3:
                continue
            if tok in {
                "the", "and", "for", "with", "that", "this", "what", "who", "when", "where",
                "from", "into", "what's", "whats", "are", "was", "were", "does", "did", "do",
                "you", "your", "session", "earlier", "first", "line", "name",
            }:
                continue
            toks.add(tok)
        return toks

    def resolve_ctc_compaction_query(self, session_id: str, query: str) -> Tuple[str, Dict[str, Any]]:
        """Deterministically resolve a session-recall query from compaction records."""
        snap = self.get_latest_ctc_compaction_snapshot(session_id)
        if not snap:
            return "", {
                "source": "",
                "field": "",
        ...
        q_tokens = self._compaction_query_tokens(q)
        if not q_tokens:
            return "", {
                "source": "",
                "field": "",
                ...
                "snapshot_found": True,
                "record_count": 1,
                "last_snapshot_id": str(snap.get("snapshot_id") or ""),
            }

        q_norm = " ".join(q.split())
        q_exact = q_norm.lower()
        sid = self._normalize_session_id(session_id)

        def _norm_text(value: Any) -> str:
            return " ".join(str(value or "").strip().lower().split())

        def _exact_session_match(text: Any) -> bool:
            t = self._compact_compaction_query_text(text, max_len=320).strip()
            if not t:
                return False
            low = _norm_text(t)
            return bool(low and (low == q_exact or low in q_exact or q_exact in low))

        def _is_low_info_literal(text: Any) -> bool:
            t = self._compact_compaction_query_text(text, max_len=80).strip()
            if not t:
                return True
            low = t.lower()
            if len(t) <= 8 and t.upper() == t and any(ch.isalpha() for ch in t):
                return True
            if low in {"must", "yes", "no", "ok", "okay", "true", "false", "n/a", "na"}:
                return True
            return False

        def _looks_like_query(text: Any) -> bool:
            t = _norm_text(text)
            if not t:
                return False
            if "?" in t:
            ...
            return any(t.startswith(prefix) for prefix in queryy)

        structured_candidates: List[Tuple[str, str, str]] = []
        anchors = snap.get("anchor_facts") or []
        for a in anchors:
            at = self._compact_compaction_query_text(a, max_len=240)
            al = at.lower()
            field = "anchor_facts"
            if al.startswith("opening line"):
                field = "anchor_opening"
            elif any(k in al for k in ("ship:", "captain:", "quarry:", "harpooner:", "pointer:")):
                field = "anchor_role"
            elif any(k in al for k in ("compaction_pressure_pct", "ctx_id", ">>summ", "vodka", "ctc", "queue")):
                field = "anchor_entity"
            structured_candidates.append(("anchor_facts", field, at))

        last_corr = snap.get("last_correction")
        if last_corr:
            structured_candidates.append(("last_correction", "last_correction", last_corr))
        last_decision = snap.get("last_decision")
        if last_decision:
            structured_candidates.append(("last_decision", "last_decision", last_decision))

        task_state = str(snap.get("task_state") or "").strip()
        if structured_candidates:
            for source, field, text in structured_candidates:
                if source == "anchor_facts" and _is_low_info_literal(text):
                    continue
                if source in {"last_correction", "last_decision"} and (_is_low_info_literal(text) or _looks_like_query(text)):
                    continue
                if _exact_session_match(text):
                    return self._compact_compaction_query_text(text, max_len=320), {
                        "source": source,
                        "field": field,
                        "reason": "exact_match",
                        "snapshot_found": True,
        ...
        # Tier 2: breadcrumb expansion from omitted spans.
        breadcrumbs_path = self._ctc_breadcrumb_read_file(session_id)
        breadcrumbs = self._load_ctc_breadcrumb_jsonl(breadcrumbs_path)
        omitted_spans = snap.get("omitted_spans") or []
        if not isinstance(omitted_spans, list):
            omitted_spans = []
        allowed_ids = {
            str(s.get("ctx_id") or "").strip()
            for s in omitted_spans
            if isinstance(s, dict)
            and str(s.get("ctx_id") or "").strip()
            and str(s.get("kind") or "").strip() in {"compaction_anchor", "history"}
        }
        breadcrumb_candidates: List[Dict[str, Any]] = []

        def _push_breadcrumb(source: str, field: str, ctx_id: str, text: Any, priority: int) -> None:
            t = self._compact_compaction_query_text(text, max_len=500).strip()
            if not t:
                return
            low = t.lower()
            if low in {"none", "null", "n/a", "na"}:
                return
            cand_tokens = self._compaction_query_tokens(low)
            if not cand_tokens:
                return
            overlap = len(q_tokens & cand_tokens)
            if overlap <= 0 and not _exact_session_match(t):
                return
            breadcrumb_candidates.append(
                {
        ...
        for rec in breadcrumbs:
            ...
            if text is None:
                continue
            _push_breadcrumb("breadcrumb", str(rec.get("kind") or "compaction_anchor"), cid, text, 250)

        if breadcrumb_candidates:
            deduped_candidates: List[Dict[str, Any]] = []
            seen_ctx_ids: set[str] = set()
            for cand in breadcrumb_candidates:
                cid = str(cand.get("ctx_id") or "").strip()
                if cid and cid in seen_ctx_ids:
                    continue
                if cid:
            ...
            if len(candidates) > 10:
                candidates.sort(key=lambda item: (-int(item.get("overlap") or 0), -int(item.get("priority") or 0), -int(item.get("tokens") or 0), len(str(item.get("text") or ""))))
                candidates = candidates[:10]

            with self._ctc_last_returned_ctx_lock:
                embed_model = get_embed_model()
                reranker = get_rerank_model()
                cosine_scores: List[float] = []
                prev_ctx_id = str(self._ctc_last_returned_ctx_by_session.get(sid) or "").strip()
                try:
                    query_vec = embed_model.encode(f"query: {q_norm}", normalize_embeddings=True)
                    passage_texts = [f"passage: {str(c.get('text') or '')}" for c in candidates]
                    passage_vecs = embed_model.encode(passage_texts, normalize_embeddings=True)
                    try:
                        passage_rows = list(passage_vecs.tolist())  # type: ignore[attr-defined]
                    except Exception:
                        passage_rows = list(passage_vecs)
                    try:
                        query_row = list(query_vec.tolist())  # type: ignore[attr-defined]
                    except Exception:
                        query_row = list(query_vec)
                    for cand, vec in zip(candidates, passage_rows):
                        score = float(sum(float(a) * float(b) for a, b in zip(query_row, vec)))
                        penalty = 0.2 if prev_ctx_id and str(cand.get("ctx_id") or "").strip() == prev_ctx_id else 0.0
                        cand["cosine_score"] = score - penalty
                    cosine_scores = [float(c.get("cosine_score") or float("-inf")) for c in candidates]
                except (AttributeError, LookupError, RuntimeError, TypeError, ValueError) as exc:
                    _LOG.warning(
                        "[ctc_breadcrumb_embed_fail] session_id=%s snapshot_id=%s err=%r",
                        session_id,
                        str(snap.get("snapshot_id") or ""),
                        exc,
                    )
                    for cand in candidates:
                        penalty = 0.2 if prev_ctx_id and str(cand.get("ctx_id") or "").strip() == prev_ctx_id else 0.0
                        cand["cosine_score"] = float(cand.get("overlap") or 0) - penalty
                    cosine_scores = [float(c.get("cosine_score") or float("-inf")) for c in candidates]

                candidates.sort(
                    key=lambda item: (
                        -float(item.get("cosine_score") or float("-inf")),
                ...
                selection_reason = "breadcrumb_e5_cosine"

                if reranker is not None and len(candidates) > 1:
                    best_score = float(selected.get("cosine_score") or float("-inf"))
                    tie_candidates = [cand for cand in candidates if best_score - float(cand.get("cosine_score") or float("-inf")) <= 0.05]
                    if len(tie_candidates) > 1:
                        try:
                            pairs = [(q_norm, str(c.get("text") or "")) for c in tie_candidates]
                            scores = reranker.predict(pairs)
                            tie_scores = list(getattr(scores, "tolist", lambda: scores)())
                            for cand, score in zip(tie_candidates, tie_scores):
                                cand["rerank_score"] = float(score)
                            tie_candidates.sort(
                                key=lambda item: (
                                    -float(item.get("rerank_score") or float("-inf")),
                                    -float(item.get("cosine_score") or float("-inf")),
                                    -int(item.get("priority") or 0),
                                    -int(item.get("overlap") or 0),
                                    len(str(item.get("text") or "")),
                            ...
                            selection_reason = "breadcrumb_e5_cosine_tiebreak_rerank"
                        except (AttributeError, LookupError, RuntimeError, TypeError, ValueError) as exc:
                            _LOG.warning(
                                "[ctc_breadcrumb_rerank_fail] session_id=%s snapshot_id=%s err=%r",
                                session_id,
                                str(snap.get("snapshot_id") or ""),
                                exc,
                            )

                selected_ctx_id = str(selected.get("ctx_id") or "").strip()
                if selected_ctx_id:
                    self._ctc_last_returned_ctx_by_session[sid] = selected_ctx_id

                source = str(selected.get("source") or "breadcrumb")
                field = str(selected.get("field") or "compaction_anchor")
                candidate_text = str(selected.get("text") or "")
                answer_text = candidate_text
                _LOG.warning(
                    "[ctc_breadcrumb_return] session_id=%s snapshot_id=%s candidates=%d prefiltered=%d selected_source=%s selected_field=%s selected_reason=%s selected_overlap=%d selected_score=%s preview=%s",
                    session_id,
                    str(snap.get("snapshot_id") or ""),
                    ...
                    field,
                    selection_reason,
                    int(selected.get("overlap") or 0),
                    str(selected.get("cosine_score") if "cosine_score" in selected else ""),
                    safe_preview(answer_text, 160),
                )
                return answer_text, {
                    "source": source,
                    "field": field,
                    ...
                    "snapshot_found": True,
                    "record_count": max(1, len(breadcrumbs)),
                    "last_snapshot_id": str(snap.get("snapshot_id") or ""),
                }

        if task_state:
            low_task_state = task_state.lower()
            if any(
                k in low_task_state
                for k in (
                    "ctc_compaction_debug",
                    ...
                    "profile_enabled=",
                    "profile_confidence=",
                    "profile_effective_strength=",
                )
            ):
                task_state = ""
            task_candidates: List[Tuple[int, int, str, str, str]] = []
            min_task_overlap = 2
            for sent in self._split_compaction_sentences(task_state):
                sl = sent.lower()
                if any(
                    k in sl
                    for k in (
                    ...
                    continue
                field = "task_sentence"
                if any(k in sl for k in ("compaction_pressure_pct", "router-config.yaml", "compaction:", "64", "256 kib", "8 kib")):
                    field = "task_cfg"
                elif any(k in sl for k in ("ctx_id", "pointer", "anchor_facts", ">>summ", "double-bang", "double-question", "vodka", "ctc")):
                    field = "task_schema"
                elif any(k in sl for k in ("recall", "retrieval", "snapshot", "boundary", "decision", "constraint")):
                    field = "task_recall"
                t = self._compact_compaction_query_text(sent, max_len=320).strip()
                cand_tokens = self._compaction_query_tokens(t.lower())
                if not cand_tokens:
                    continue
                overlap = len(q_tokens & cand_tokens)
                if overlap < min_task_overlap and not _exact_session_match(t):
                    continue
                score = 100 + overlap * 10 + min(40, len(cand_tokens))
                task_candidates.append((score, overlap, "task_state", field, t))
            if task_candidates:
                task_candidates.sort(key=lambda item: (-item[0], -item[1], len(item[4])))
                _, _, source, field, text = task_candidates[0]
                return text, {
                    "source": source,
                    "field": field,
                    "reason": "task_state_match",
                    "snapshot_found": True,
                    "record_count": 1,
                    "last_snapshot_id": str(snap.get("snapshot_id") or ""),
                }

        if not structured_candidates and not breadcrumb_candidates:
            return "", {
                "source": "",
                "field": "",
                "reason": "no_overlap",
    ...
    def _clip_and_trim_messages(
        ...
        *,
        ...
        units_after_window = len([m for m in out if m.get("role") in ("user", "assistant")])
        if units_after_window < units_before:
            chars_before = sum(len(str(m.get("content", "") or "")) for m in ua)
            chars_after_window = sum(
            ...
            _append_trim_event(
                ...
                units_after=units_after_window,
                chars_trimmed=max(0, chars_before - chars_after_window),
            )
        ...
        if max_chars and max_chars > 0:
            ...
            units_after = len([m for m in selected if m.get("role") in ("user", "assistant")])
            if units_after < units_before:
                ...
                _append_trim_event(
                    ...
                    chars_trimmed=max(0, chars_before - chars_after),
                )
    ...
    def inlet(self, body: dict, user: Optional[dict] = None) -> dict:
        ...
        session_id = str(body.get("session_id") or "").strip() or "global"
        print(f"[ctc_inlet_called] session_id={session_id}", flush=True)
        ctx_size = int(body.get("ctx_size") or 0)
        current_turn = int(body.get("_current_turn", 0) or 0)
        ...
        # 1b) ingestion-time breadcrumb capture for structured session chunks.
        last_user_turn = ""
        for m in reversed(messages):
            ...
            c = m.get("content", "")
            if isinstance(c, str) and c.strip():
                last_user_turn = c.strip()
                break
        if last_user_turn.startswith("[SESSION-CHUNK-"):
            # Author guard: user-authored structured chunks are not stored as breadcrumbs.
            pass

        # 2) canonical memory pipeline (capture -> retrieve -> render/inject)
        current_mode = str(body.get("mode") or body.get("recall_mode") or body.get("request_mode") or "").strip() or "unknown"
        _LOG.warning(
            "[ctc_gate_check] session_id=%s enable_summary=%s mode=%s",
            session_id,
            bool(self.valves.enable_summary),
            current_mode,
        )
        ...
        # 3) true compaction boundary snapshot (CTC lane)
        _LOG.warning("[ctc_inlet_reach] session_id=%s enable_summary=%s", session_id, bool(self.valves.enable_summary))
        messages, _did_compact = self._maybe_compact_ctc_history(
            messages,
            session_id=session_id,
            ctx_size=ctx_size,
            current_turn=current_turn,
        ...
        # 4) bounded clip as a final safety rail (no first-turn special case).
