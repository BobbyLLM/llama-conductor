...
MODEL_REQUEST_TRACE: ContextVar[Dict[str, Any]] = ContextVar("MODEL_REQUEST_TRACE", default={})
_ROUTER_VERSION_CACHE: Optional[str] = None
...
def _router_version() -> str:
    global _ROUTER_VERSION_CACHE
    if _ROUTER_VERSION_CACHE is not None:
        return _ROUTER_VERSION_CACHE
    version = ""
    try:
        ...
        if pyproject.exists():
            for ln in pyproject.read_text(encoding="utf-8").splitlines():
                ...
                if s.startswith("version") and "=" in s:
                    _, rhs = s.split("=", 1)
                    cand = str(rhs or "").strip().strip('"').strip("'")
                    if cand:
                        version = cand
                        break
    except Exception:
        version = ""
    _ROUTER_VERSION_CACHE = version or "unknown"
    return _ROUTER_VERSION_CACHE


def _append_request_trace(record: Dict[str, Any]) -> None:
    try:
        log_path = Path(__file__).resolve().parents[1] / "logs" / "model_request.log"
        log_path.parent.mkdir(parents=True, exist_ok=True)
        stamp = datetime.now(timezone.utc).isoformat()
        with log_path.open("a", encoding="utf-8") as fh:
            fh.write(f"{stamp} {json.dumps(record, ensure_ascii=False, sort_keys=True)}\n")
    except Exception:
        pass


def _build_request_trace(*, kind: str, role: str, payload: Dict[str, Any], debug_context: str = "") -> Dict[str, Any]:
    trace_meta = dict(MODEL_REQUEST_TRACE.get() or {})
    params = {
    ...
    return {
        "kind": kind,
        "role": role,
        "endpoint_url": UPSTREAM_CHAT_URL,
...
def call_model_prompt(
    *,
    ...
    try:
        _append_request_trace(_build_request_trace(kind="prompt", role=role, payload=payload, debug_context=debug_context))
...
def call_model_messages(
    *,
    ...
    try:
        _append_request_trace(_build_request_trace(kind="messages", role=role, payload=payload))
