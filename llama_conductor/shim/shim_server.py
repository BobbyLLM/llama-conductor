...
_SHIM_DEBUG_LOGS: List[Dict[str, Any]] = []
_SHIM_DEBUG_LOGS_LIMIT = 200
...
(function () {
  ...
  function appendAssistantToCurrentChat(content) {
    ...
    if (!text) return;
    if (!isCompleteAssistantText(text)) return;
    var state = captureState();
  ...
  function isCompleteAssistantText(text) {
    ...
    var lastWord = words[words.length - 1].toLowerCase().replace(/[^a-z]/g, "");
    var danglers = new Set(["on", "and", "but", "the", "a", "an", "or", "in", "at", "to", "of", "with", "for", "from", "by"]);
    if (danglers.has(lastWord)) return false;
    return /[.?!\"']$/.test(content);
  }

  function emitShimDebug(event) {
    try {
      if (!originalFetch) return;
      var payload = event && typeof event === "object" ? event : { value: event };
      payload.ts = Date.now();
      payload.chat_key = currentChatKey();
      payload.ua = String((navigator && navigator.userAgent) || "");
      originalFetch("/shim/debug/log", {
        method: "POST",
  ...
  function extractAssistantCaptureFromJson(payload) {
    var capture = { text: "", complete: false, sawDone: null, sawTerminalChunk: null };
    if (!payload || typeof payload !== "object") return capture;
    if (Array.isArray(payload.choices) && payload.choices.length > 0) {
      var choice = payload.choices[0] || {};
      if (choice.message && choice.message.content != null) {
        capture.text = String(choice.message.content);
        capture.complete = String(choice.finish_reason || "").toLowerCase() !== "error";
        return capture;
      }
      if (choice.delta && choice.delta.content != null) {
        capture.text = String(choice.delta.content);
        capture.complete = String(choice.finish_reason || "").toLowerCase() === "stop";
        return capture;
      }
      if (choice.text != null) {
        capture.text = String(choice.text);
        capture.complete = String(choice.finish_reason || "").toLowerCase() !== "error";
        return capture;
      }
    }
    if (payload.message && payload.message.content != null) {
      capture.text = String(payload.message.content);
      capture.complete = true;
      return capture;
    }
    if (payload.content != null) {
      capture.text = String(payload.content);
      capture.complete = true;
      return capture;
    }
    return capture;
  ...
  function parseSseAssistantCapture(raw) {
    var capture = { text: "", complete: false, sawDone: false, sawTerminalChunk: false };
    if (typeof raw !== "string" || raw.length === 0) return capture;
    var out = [];
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      if (Array.isArray(obj.choices)) {
        for (var j = 0; j < obj.choices.length; j++) {
          ...
          if (String(ch.finish_reason || "").toLowerCase() === "stop") sawTerminalChunk = true;
        }
      } else if (obj.content != null) {
        out.push(String(obj.content));
      }
    }
    capture.text = out.join("");
    capture.complete = sawDone && sawTerminalChunk;
    return capture;
  }

  function isFirefoxShimClient() {
    try {
      return /firefox/i.test(String((navigator && navigator.userAgent) || "")) && !/seamonkey/i.test(String((navigator && navigator.userAgent) || ""));
    } catch (_err) {
      return false;
    }
  }
      ...
      if (isChatCall && method === "POST" && response && response.ok) {
        (async function (resp) {
          try {
            ...
            var assistant = null;
            var assistantComplete = false;

            if (contentType.indexOf("text/event-stream") >= 0) {
              var sseText = await clone.text();
              var sseCapture = parseSseAssistantCapture(sseText);
              assistant = sseCapture.text;
              assistantComplete = sseCapture.complete;
              emitShimDebug({
                kind: "assistant_commit_probe",
                transport: "sse",
                assistantComplete: assistantComplete,
                sawDone: sseCapture.sawDone,
                sawTerminalChunk: sseCapture.sawTerminalChunk,
                content: assistant == null ? "" : String(assistant),
                length: assistant == null ? 0 : String(assistant).length,
                committed: !isFirefoxShimClient() || assistantComplete
              });
            } else {
              var rawText = await clone.text();
              var parsed = tryParseJson(rawText);
              var jsonCapture = extractAssistantCaptureFromJson(parsed);
              assistant = jsonCapture.text;
              assistantComplete = jsonCapture.complete;
              if ((assistant == null || assistant === "") && rawText && rawText.indexOf("data:") >= 0) {
                var rawCapture = parseSseAssistantCapture(rawText);
                assistant = rawCapture.text;
                assistantComplete = rawCapture.complete;
                emitShimDebug({
                  kind: "assistant_commit_probe",
                  transport: "sse-in-json",
                  assistantComplete: assistantComplete,
                  sawDone: rawCapture.sawDone,
                  sawTerminalChunk: rawCapture.sawTerminalChunk,
                  content: assistant == null ? "" : String(assistant),
                  length: assistant == null ? 0 : String(assistant).length,
                  committed: !isFirefoxShimClient() || assistantComplete
                });
              } else if (assistant != null && String(assistant).trim() !== "") {
                emitShimDebug({
                  kind: "assistant_commit_probe",
                  transport: "json",
                  assistantComplete: assistantComplete,
                  sawDone: jsonCapture.sawDone,
                  sawTerminalChunk: jsonCapture.sawTerminalChunk,
                  content: assistant == null ? "" : String(assistant),
                  length: assistant == null ? 0 : String(assistant).length,
                  committed: !isFirefoxShimClient() || assistantComplete
                });
            ...
            if (assistant != null && String(assistant).trim() !== "") {
              if (!isFirefoxShimClient() || assistantComplete) {
                appendAssistantToCurrentChat(assistant);
              }
            }
          } catch (_err) {}
        })(response);
...
@APP.post("/shim/debug/log")
async def shim_debug_log(request: Request):
    ...
    _SHIM_DEBUG_LOGS.append(entry)
    if len(_SHIM_DEBUG_LOGS) > _SHIM_DEBUG_LOGS_LIMIT:
      del _SHIM_DEBUG_LOGS[: len(_SHIM_DEBUG_LOGS) - _SHIM_DEBUG_LOGS_LIMIT]
    return JSONResponse({"ok": True})


@APP.get("/shim/debug/log")
async def shim_debug_log_get():
    return JSONResponse(_SHIM_DEBUG_LOGS)


@APP.delete("/shim/debug/log")
async def shim_debug_log_clear():
    _SHIM_DEBUG_LOGS.clear()
    return JSONResponse({"ok": True})
