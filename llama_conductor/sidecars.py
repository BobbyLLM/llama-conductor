...
_WIKI_STOPWORDS = {
    ...
    "could", "might", "would", "should", "shall", "will", "may", "must",
}
_WIKI_FOCUS_BREAK_TOKENS = {
    "have", "has", "had", "been", "be", "being", "was", "were",
    "do", "does", "did", "can", "could", "would", "should", "might", "may", "will", "shall",
    "to", "than", "if", "before", "after", "earlier", "later", "when", "where", "why", "how",
}
...
def _wiki_focus_query(topic: str) -> str:
    raw = re.sub(r"\s+", " ", str(topic or "").strip())
    if not raw:
        return ""
    shape = classify_question_shape(raw)
    if shape.kind != "analogical" and _LEADING_MODAL_SCAFFOLD_RE.search(raw):
        stripped = _LEADING_MODAL_SCAFFOLD_RE.sub("", raw).strip()
        if stripped:
            toks = [t for t in _WIKI_TOKEN_RE.findall(stripped) if t]
            anchor: list[str] = []
            for tok in toks:
                low = tok.lower()
                if not anchor and (low in _WIKI_STOPWORDS):
                    continue
                if low in _WIKI_FOCUS_BREAK_TOKENS:
                    break
                if low in _WIKI_STOPWORDS:
                    if anchor:
                        break
                    continue
                anchor.append(tok)
...
def handle_wiki_query(topic: str, max_chars: int = 800) -> str:
    ...
    headers = {"User-Agent": "llama-conductor/1.0.1"}
    query_topic = _wiki_focus_query(topic) or topic
    normalized = _normalize_wiki_topic(query_topic)
    ...
    try:
        ...
        if status == 404:
            best = _wiki_search_best_title(query_topic)
            if best:
...
def resolve_web_evidence(query: str) -> Tuple[bool, Dict[str, Any], str]:
    ...
    try:
        ...
        policy = _web_synth_load_class_policy("")
        ...
        for h in filtered:
            host = _web_domain(str(h.url or "").strip())
            if not host:
                continue
