...
TL;DR: CTC now has a real breadcrumb-backed compaction path. When the live chat gets too big, the router trims the oversized window, writes compact JSONL records to disk, and later rehydrates the saved notes instead of guessing from whatever text happens to remain in the prompt.

- What compaction is:
  - the router’s cleanup pass for long chats
  - it keeps the important bits and trims the rest before the prompt gets too big
  - compaction snapshots live under `total_recall/vodka/ctc_compaction/`
  - breadcrumb records live under `total_recall/vodka/ctc_breadcrumbs/`
- How it works:
  - each focused breadcrumb is written as its own JSONL record
  - compaction snapshots store `omitted_spans` pointers to the matching breadcrumb records
  - recall questions re-open the snapshot, follow the pointers, and rank the candidate notes
  - e5 cosine is the primary scorer, with TinyBERT used as a tie-breaker when candidates are close
- How it is rehydrated:
  - the router reads the JSONL snapshot on recall
  - it resolves the saved breadcrumb IDs from `omitted_spans`
  - it returns the best matching saved note instead of rebuilding the answer from raw context
- Validation:
  - validated across 45 live smoke runs
  - exact structured smoke passes `5/5`
  - router-log inspection confirms the right breadcrumb records are actually being selected
  - paraphrase / negative-control verification passes `4/5`
- Known edge cases:
   - a dense generic breadcrumb can still win if the question is too broad


### 1.9.5

TL;DR: Codex grounding shipped. The router can now answer from SUMM-backed source text through a real grounded wiki path, with deterministic rebuilds and live reranking, instead of relying on model memory alone.

Origin of the idea: https://tinyurl.com/llmwiki

- Codex grounding integration:
  - added the Codex retrieval runtime in `llama_conductor/codex_runtime.py`
  - Codex rebuilds are driven by deterministic Python tooling, not an LLM summary pass
  - source material is taken from `SUMM_*.md` files and related indexed pages
  - `raw_text` is preserved in the Codex payload so grounding uses the original summary content, not only the rendered wiki page
  - grounded facts are injected through `facts_block`
  - router mode handling applies Codex grounding consistently across the supported answer styles

- Retrieval behavior:
  - e5 selects candidate material first
  - TinyBERT reranks the shortlisted candidates
  - Codex answers are grounded in the local knowledge base, with explicit fallback when nothing relevant is found

- Validation:
  - live smoke runs confirmed the end-to-end Codex wiring
  - grounded answers now come from the SUMM-backed path instead of model priors
